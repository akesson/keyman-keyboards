KeymanWeb.KR(new Keyboard_pashtokeyboard());

function Keyboard_pashtokeyboard() {
    this.KI = "Keyboard_pashtokeyboard";
    this.KN = "Pashto Standard Keyboard";
    this.KV = {
        F: ' 1em "Pak Nastaleeq"',
        K102: 0,
        BK: new Array("‍", "۱", "۲", "۳", "۴", "۵", "۶", "۷", "۸", "۹", "۰", "-", "=", "", "", "", "ض", "ص", "ث", "ق", "ف", "غ", "ع", "ه", "خ", "ح", "ج", "چ", "\\", "", "", "", "ش", "س", "ی", "ب", "ل", "ا", "ت", "ن", "م", "ک", "ګ", "", "", "", "", "", "", "ظ", "ط", "ز", "ر", "ذ", "د", "ړ", "و", "ږ", "/", "", "", "", "", "", "", "ٔ", "!", "٬", "٫", "؋", "٪", "×", "»", "«", ")", "(", "ـ", "+", "", "", "", "ْ", "ٌ", "ٍ", "ً", "ُ", "ِ", "َ", "ّ", "ځ", "څ", "]", "[", "*", "", "", "", "ښ", "ۍ", "ي", "پ", "أ", "آ", "ټ", "ڼ", "ة", ":", "؛", "", "", "", "", "", "", "ئ", "ې", "ژ", "ء", "‌", "ډ", "ؤ", "،", ".", "؟", "", "", "", "", "", "", "", "", "", "", "", "", "", "", "", "", "", "", "", "", "", "", "", "", "", "", "", "", "", "", "", "", "", "", "", "", "", "", "", "", "", "", "", "", "", "", "", "", "", "", "", "", "", "", "", "", "", "", "", "", "", "", "", "", "", "", "", "", "", "", "", "", "", "", "", "", "", "", "", "", "", "", "", "", "", "", "", "", "", "", "", "", "", "", "", "", "", "", "", "", "", "", "", "", "", "", "", "", "", "", "", "", "", "", "", "", "", "", "", "", "", "", "", "", "", "", "", "", "", "", "", "", "", "", "", "", "", "يو ", "دوه ", "دری ", "څلور ", "پنځه ", "شپږ ", "اووه ", "اته ", "نه ", "صفر ", "لس ", "مليون ", "", "", "", "اوس ", "وال ", "سره ", "خپل ", "کوي ", "کله ", "شوی ", "هغه ", "ځخه ", "ستا ", "موږ ", "زما ", "", "", "", "", "دا ", "ده ", "دي ", "دی ", "يا ", "له ", "هم ", "نه ", "چی ", "تر ", "په ", "", "", "", "", "", "", "افغانستان ", "افغان ", "پښتو ", "هېواد ", "اسلام ", "کېږي ", "لپاره ", "کېدل ", "شي ", "لوی ", "", "", "", "", "", "")
    };
    this.KH = '';
    this.KM = 0;
    this.KRTL = 1;
    this.gs = function(t, e) {
        return this.g0(t, e);
    };
    this.g0 = function(t, e) {
        var k = KeymanWeb,
            r = 0,
            m = 0;
        if (k.KKM(e, 16448, 48)) {
            r = m = 1;
            k.KO(0, t, "صفر ");
        } else if (k.KKM(e, 16400, 48)) {
            r = m = 1;
            k.KO(0, t, "(");
        } else if (k.KKM(e, 16384, 48)) {
            r = m = 1;
            k.KO(0, t, "۰");
        } else if (k.KKM(e, 16448, 49)) {
            r = m = 1;
            k.KO(0, t, "يو ");
        } else if (k.KKM(e, 16400, 49)) {
            r = m = 1;
            k.KO(0, t, "!");
        } else if (k.KKM(e, 16384, 49)) {
            r = m = 1;
            k.KO(0, t, "۱");
        } else if (k.KKM(e, 16448, 50)) {
            r = m = 1;
            k.KO(0, t, "دوه ");
        } else if (k.KKM(e, 16400, 50)) {
            r = m = 1;
            k.KO(0, t, "٬");
        } else if (k.KKM(e, 16384, 50)) {
            r = m = 1;
            k.KO(0, t, "۲");
        } else if (k.KKM(e, 16448, 51)) {
            r = m = 1;
            k.KO(0, t, "دری ");
        } else if (k.KKM(e, 16400, 51)) {
            r = m = 1;
            k.KO(0, t, "٫");
        } else if (k.KKM(e, 16384, 51)) {
            r = m = 1;
            k.KO(0, t, "۳");
        } else if (k.KKM(e, 16448, 52)) {
            r = m = 1;
            k.KO(0, t, "څلور ");
        } else if (k.KKM(e, 16400, 52)) {
            r = m = 1;
            k.KO(0, t, "؋");
        } else if (k.KKM(e, 16384, 52)) {
            r = m = 1;
            k.KO(0, t, "۴");
        } else if (k.KKM(e, 16448, 53)) {
            r = m = 1;
            k.KO(0, t, "پنځه ");
        } else if (k.KKM(e, 16400, 53)) {
            r = m = 1;
            k.KO(0, t, "٪");
        } else if (k.KKM(e, 16384, 53)) {
            r = m = 1;
            k.KO(0, t, "۵");
        } else if (k.KKM(e, 16448, 54)) {
            r = m = 1;
            k.KO(0, t, "شپږ ");
        } else if (k.KKM(e, 16400, 54)) {
            r = m = 1;
            k.KO(0, t, "×");
        } else if (k.KKM(e, 16384, 54)) {
            r = m = 1;
            k.KO(0, t, "۶");
        } else if (k.KKM(e, 16448, 55)) {
            r = m = 1;
            k.KO(0, t, "اووه ");
        } else if (k.KKM(e, 16400, 55)) {
            r = m = 1;
            k.KO(0, t, "»");
        } else if (k.KKM(e, 16384, 55)) {
            r = m = 1;
            k.KO(0, t, "۷");
        } else if (k.KKM(e, 16448, 56)) {
            r = m = 1;
            k.KO(0, t, "اته ");
        } else if (k.KKM(e, 16400, 56)) {
            r = m = 1;
            k.KO(0, t, "«");
        } else if (k.KKM(e, 16384, 56)) {
            r = m = 1;
            k.KO(0, t, "۸");
        } else if (k.KKM(e, 16448, 57)) {
            r = m = 1;
            k.KO(0, t, "نه ");
        } else if (k.KKM(e, 16400, 57)) {
            r = m = 1;
            k.KO(0, t, ")");
        } else if (k.KKM(e, 16384, 57)) {
            r = m = 1;
            k.KO(0, t, "۹");
        } else if (k.KKM(e, 16448, 65)) {
            r = m = 1;
            k.KO(0, t, "دا ");
        } else if (k.KKM(e, 16400, 65)) {
            r = m = 1;
            k.KO(0, t, "ښ");
        } else if (k.KKM(e, 16384, 65)) {
            r = m = 1;
            k.KO(0, t, "ش");
        } else if (k.KKM(e, 16448, 66)) {
            r = m = 1;
            k.KO(0, t, "اسلام ");
        } else if (k.KKM(e, 16400, 66)) {
            r = m = 1;
            k.KO(0, t, "‌");
        } else if (k.KKM(e, 16384, 66)) {
            r = m = 1;
            k.KO(0, t, "ذ");
        } else if (k.KKM(e, 16448, 67)) {
            r = m = 1;
            k.KO(0, t, "پښتو ");
        } else if (k.KKM(e, 16400, 67)) {
            r = m = 1;
            k.KO(0, t, "ژ");
        } else if (k.KKM(e, 16384, 67)) {
            r = m = 1;
            k.KO(0, t, "ز");
        } else if (k.KKM(e, 16448, 68)) {
            r = m = 1;
            k.KO(0, t, "دي ");
        } else if (k.KKM(e, 16400, 68)) {
            r = m = 1;
            k.KO(0, t, "ي");
        } else if (k.KKM(e, 16384, 68)) {
            r = m = 1;
            k.KO(0, t, "ی");
        } else if (k.KKM(e, 16448, 69)) {
            r = m = 1;
            k.KO(0, t, "سره ");
        } else if (k.KKM(e, 16400, 69)) {
            r = m = 1;
            k.KO(0, t, "ٍ");
        } else if (k.KKM(e, 16384, 69)) {
            r = m = 1;
            k.KO(0, t, "ث");
        } else if (k.KKM(e, 16448, 70)) {
            r = m = 1;
            k.KO(0, t, "دی ");
        } else if (k.KKM(e, 16400, 70)) {
            r = m = 1;
            k.KO(0, t, "پ");
        } else if (k.KKM(e, 16384, 70)) {
            r = m = 1;
            k.KO(0, t, "ب");
        } else if (k.KKM(e, 16448, 71)) {
            r = m = 1;
            k.KO(0, t, "يا ");
        } else if (k.KKM(e, 16400, 71)) {
            r = m = 1;
            k.KO(0, t, "أ");
        } else if (k.KKM(e, 16384, 71)) {
            r = m = 1;
            k.KO(0, t, "ل");
        } else if (k.KKM(e, 16448, 72)) {
            r = m = 1;
            k.KO(0, t, "له ");
        } else if (k.KKM(e, 16400, 72)) {
            r = m = 1;
            k.KO(0, t, "آ");
        } else if (k.KKM(e, 16384, 72)) {
            r = m = 1;
            k.KO(0, t, "ا");
        } else if (k.KKM(e, 16448, 73)) {
            r = m = 1;
            k.KO(0, t, "هغه ");
        } else if (k.KKM(e, 16400, 73)) {
            r = m = 1;
            k.KO(0, t, "ّ");
        } else if (k.KKM(e, 16384, 73)) {
            r = m = 1;
            k.KO(0, t, "ه");
        } else if (k.KKM(e, 16448, 74)) {
            r = m = 1;
            k.KO(0, t, "هم ");
        } else if (k.KKM(e, 16400, 74)) {
            r = m = 1;
            k.KO(0, t, "ټ");
        } else if (k.KKM(e, 16384, 74)) {
            r = m = 1;
            k.KO(0, t, "ت");
        } else if (k.KKM(e, 16448, 75)) {
            r = m = 1;
            k.KO(0, t, "نه ");
        } else if (k.KKM(e, 16400, 75)) {
            r = m = 1;
            k.KO(0, t, "ڼ");
        } else if (k.KKM(e, 16384, 75)) {
            r = m = 1;
            k.KO(0, t, "ن");
        } else if (k.KKM(e, 16448, 76)) {
            r = m = 1;
            k.KO(0, t, "چی ");
        } else if (k.KKM(e, 16400, 76)) {
            r = m = 1;
            k.KO(0, t, "ة");
        } else if (k.KKM(e, 16384, 76)) {
            r = m = 1;
            k.KO(0, t, "م");
        } else if (k.KKM(e, 16448, 77)) {
            r = m = 1;
            k.KO(0, t, "لپاره ");
        } else if (k.KKM(e, 16400, 77)) {
            r = m = 1;
            k.KO(0, t, "ؤ");
        } else if (k.KKM(e, 16384, 77)) {
            r = m = 1;
            k.KO(0, t, "ړ");
        } else if (k.KKM(e, 16448, 78)) {
            r = m = 1;
            k.KO(0, t, "کېږي ");
        } else if (k.KKM(e, 16400, 78)) {
            r = m = 1;
            k.KO(0, t, "ډ");
        } else if (k.KKM(e, 16384, 78)) {
            r = m = 1;
            k.KO(0, t, "د");
        } else if (k.KKM(e, 16448, 79)) {
            r = m = 1;
            k.KO(0, t, "ځخه ");
        } else if (k.KKM(e, 16400, 79)) {
            r = m = 1;
            k.KO(0, t, "ځ");
        } else if (k.KKM(e, 16384, 79)) {
            r = m = 1;
            k.KO(0, t, "خ");
        } else if (k.KKM(e, 16448, 80)) {
            r = m = 1;
            k.KO(0, t, "ستا ");
        } else if (k.KKM(e, 16400, 80)) {
            r = m = 1;
            k.KO(0, t, "څ");
        } else if (k.KKM(e, 16384, 80)) {
            r = m = 1;
            k.KO(0, t, "ح");
        } else if (k.KKM(e, 16448, 81)) {
            r = m = 1;
            k.KO(0, t, "اوس ");
        } else if (k.KKM(e, 16400, 81)) {
            r = m = 1;
            k.KO(0, t, "ْ");
        } else if (k.KKM(e, 16384, 81)) {
            r = m = 1;
            k.KO(0, t, "ض");
        } else if (k.KKM(e, 16448, 82)) {
            r = m = 1;
            k.KO(0, t, "خپل ");
        } else if (k.KKM(e, 16400, 82)) {
            r = m = 1;
            k.KO(0, t, "ً");
        } else if (k.KKM(e, 16384, 82)) {
            r = m = 1;
            k.KO(0, t, "ق");
        } else if (k.KKM(e, 16448, 83)) {
            r = m = 1;
            k.KO(0, t, "ده ");
        } else if (k.KKM(e, 16400, 83)) {
            r = m = 1;
            k.KO(0, t, "ۍ");
        } else if (k.KKM(e, 16384, 83)) {
            r = m = 1;
            k.KO(0, t, "س");
        } else if (k.KKM(e, 16448, 84)) {
            r = m = 1;
            k.KO(0, t, "کوي ");
        } else if (k.KKM(e, 16400, 84)) {
            r = m = 1;
            k.KO(0, t, "ُ");
        } else if (k.KKM(e, 16384, 84)) {
            r = m = 1;
            k.KO(0, t, "ف");
        } else if (k.KKM(e, 16448, 85)) {
            r = m = 1;
            k.KO(0, t, "شوی ");
        } else if (k.KKM(e, 16400, 85)) {
            r = m = 1;
            k.KO(0, t, "َ");
        } else if (k.KKM(e, 16384, 85)) {
            r = m = 1;
            k.KO(0, t, "ع");
        } else if (k.KKM(e, 16448, 86)) {
            r = m = 1;
            k.KO(0, t, "هېواد ");
        } else if (k.KKM(e, 16400, 86)) {
            r = m = 1;
            k.KO(0, t, "ء");
        } else if (k.KKM(e, 16384, 86)) {
            r = m = 1;
            k.KO(0, t, "ر");
        } else if (k.KKM(e, 16448, 87)) {
            r = m = 1;
            k.KO(0, t, "وال ");
        } else if (k.KKM(e, 16400, 87)) {
            r = m = 1;
            k.KO(0, t, "ٌ");
        } else if (k.KKM(e, 16384, 87)) {
            r = m = 1;
            k.KO(0, t, "ص");
        } else if (k.KKM(e, 16448, 88)) {
            r = m = 1;
            k.KO(0, t, "افغان ");
        } else if (k.KKM(e, 16400, 88)) {
            r = m = 1;
            k.KO(0, t, "ې");
        } else if (k.KKM(e, 16384, 88)) {
            r = m = 1;
            k.KO(0, t, "ط");
        } else if (k.KKM(e, 16448, 89)) {
            r = m = 1;
            k.KO(0, t, "کله ");
        } else if (k.KKM(e, 16400, 89)) {
            r = m = 1;
            k.KO(0, t, "ِ");
        } else if (k.KKM(e, 16384, 89)) {
            r = m = 1;
            k.KO(0, t, "غ");
        } else if (k.KKM(e, 16448, 90)) {
            r = m = 1;
            k.KO(0, t, "افغانستان ");
        } else if (k.KKM(e, 16400, 90)) {
            r = m = 1;
            k.KO(0, t, "ئ");
        } else if (k.KKM(e, 16384, 90)) {
            r = m = 1;
            k.KO(0, t, "ظ");
        } else if (k.KKM(e, 16448, 186)) {
            r = m = 1;
            k.KO(0, t, "تر ");
        } else if (k.KKM(e, 16400, 186)) {
            r = m = 1;
            k.KO(0, t, ":");
        } else if (k.KKM(e, 16384, 186)) {
            r = m = 1;
            k.KO(0, t, "ک");
        } else if (k.KKM(e, 16448, 187)) {
            r = m = 1;
            k.KO(0, t, "مليون ");
        } else if (k.KKM(e, 16400, 187)) {
            r = m = 1;
            k.KO(0, t, "+");
        } else if (k.KKM(e, 16384, 187)) {
            r = m = 1;
            k.KO(0, t, "=");
        } else if (k.KKM(e, 16448, 188)) {
            r = m = 1;
            k.KO(0, t, "کېدل ");
        } else if (k.KKM(e, 16400, 188)) {
            r = m = 1;
            k.KO(0, t, "،");
        } else if (k.KKM(e, 16384, 188)) {
            r = m = 1;
            k.KO(0, t, "و");
        } else if (k.KKM(e, 16448, 189)) {
            r = m = 1;
            k.KO(0, t, "لس ");
        } else if (k.KKM(e, 16400, 189)) {
            r = m = 1;
            k.KO(0, t, "ـ");
        } else if (k.KKM(e, 16384, 189)) {
            r = m = 1;
            k.KO(0, t, "-");
        } else if (k.KKM(e, 16448, 190)) {
            r = m = 1;
            k.KO(0, t, "شي ");
        } else if (k.KKM(e, 16400, 190)) {
            r = m = 1;
            k.KO(0, t, ".");
        } else if (k.KKM(e, 16384, 190)) {
            r = m = 1;
            k.KO(0, t, "ږ");
        } else if (k.KKM(e, 16448, 191)) {
            r = m = 1;
            k.KO(0, t, "لوی ");
        } else if (k.KKM(e, 16400, 191)) {
            r = m = 1;
            k.KO(0, t, "؟");
        } else if (k.KKM(e, 16384, 191)) {
            r = m = 1;
            k.KO(0, t, "/");
        } else if (k.KKM(e, 16384, 192)) {
            r = m = 1;
            k.KO(0, t, "‍");
        } else if (k.KKM(e, 16400, 192)) {
            r = m = 1;
            k.KO(0, t, "ٔ");
        } else if (k.KKM(e, 16448, 219)) {
            r = m = 1;
            k.KO(0, t, "موږ ");
        } else if (k.KKM(e, 16400, 219)) {
            r = m = 1;
            k.KO(0, t, "]");
        } else if (k.KKM(e, 16384, 219)) {
            r = m = 1;
            k.KO(0, t, "ج");
        } else if (k.KKM(e, 16400, 220)) {
            r = m = 1;
            k.KO(0, t, "*");
        } else if (k.KKM(e, 16384, 220)) {
            r = m = 1;
            k.KO(0, t, "\\");
        } else if (k.KKM(e, 16448, 221)) {
            r = m = 1;
            k.KO(0, t, "زما ");
        } else if (k.KKM(e, 16400, 221)) {
            r = m = 1;
            k.KO(0, t, "[");
        } else if (k.KKM(e, 16384, 221)) {
            r = m = 1;
            k.KO(0, t, "چ");
        } else if (k.KKM(e, 16448, 222)) {
            r = m = 1;
            k.KO(0, t, "په ");
        } else if (k.KKM(e, 16400, 222)) {
            r = m = 1;
            k.KO(0, t, "؛");
        } else if (k.KKM(e, 16384, 222)) {
            r = m = 1;
            k.KO(0, t, "ګ");
        }
        return r;
    };
}
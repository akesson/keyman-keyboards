KeymanWeb.KR(new Keyboard_gff_sgw_7());

function Keyboard_gff_sgw_7() {
    this.KI = "Keyboard_gff_sgw_7";
    this.KN = "Sebatbeit";
    this.KDU = 1;
    this.KV = {
        F: ' 1em "Abyssinica SIL"',
        K102: 0,
        BK: new Array("`", "1", "2", "3", "4", "5", "6", "7", "8", "9", "0", "-", "=", "", "", "", "ቅⷅ", "ው", "እ", "ር", "ት", "ይ", "ኡ", "ኢ", "ኦ", "ፕ", "[", "]", "\\", "", "", "", "አ", "ስ", "ድ", "ፍ", "ግⷝ", "ኽⷕ", "ጅ", "ክⷍ", "ል", "፤", "'", "", "", "", "", "", "", "ዝ", "ሽ", "ች", "ብ", "ብ", "ን", "ም", "፣፥", ".", "/", "", "", "", "", "", "", "~", "!", "@", "#", "$", "%", "^", "&", "*", "(", ")", "፟", "+", "", "", "", "ቅⷅ", "ው", "እ", "ር", "ጥ", "ይ", "ኡ", "ኢ", "ኦ", "ጵ", "{", "}", "|", "", "", "", "አ", "ፅ", "ድ", "ፍ", "ግⷝ", "ኽⷕ", "ጅ", "ክⷍ", "ል", "፤", "'", "", "", "", "", "", "", "ዥ", "ሽ", "ጭ", "ብ", "ብ", "ኝ", "ም", "‹", "›", "?", "", "", "", "", "", "፡")
    };
    this.KH = '';
    this.KM = 0;
    this.s10 = "ለመረሰሸቀⷀበተቸነኘከⷈኸⷐወዘዠየደጀገⷘጠጨጰፀፈፐ";
    this.s11 = "ሉሙሩሱሹቁⷁቡቱቹኑኙኩⷉኹⷑዉዙዡዩዱጁጉⷙጡጩጱፁፉፑ";
    this.s12 = "ሊሚሪሲሺቂⷂቢቲቺኒኚኪⷊኺⷒዊዚዢዪዲጂጊⷚጢጪጲፂፊፒ";
    this.s13 = "ላማራሳሻቃⷃባታቻናኛካⷋኻⷓዋዛዣያዳጃጋⷛጣጫጳፃፋፓ";
    this.s14 = "ሌሜሬሴሼቄⷄቤቴቼኔኜኬⷌኼⷔዌዜዤዬዴጄጌⷜጤጬጴፄፌፔ";
    this.s15 = "ልምርስሽቅⷅብትችንኝክⷍኽⷕውዝዥይድጅግⷝጥጭጵፅፍፕ";
    this.s16 = "ሎሞሮሶሾቆⷆቦቶቾኖኞኮⷎኾⷖዎዞዦዮዶጆጎⷞጦጮጶፆፎፖ";
    this.s17 = "lmrsxq_btcnNk_h_wzZydjg_TCPSfp";
    this.s18 = "HLMRXBVvQKWYDJGF";
    this.s19 = "ኽልምርሽብብብቅክውይድጅግፍ";
    this.s20 = "ኽቅክግ";
    this.s21 = "ⷕⷅⷍⷝ";
    this.s22 = "yY";
    this.s23 = "aAuUiIeEoO";
    this.s24 = "አአኡኡኢኢእእኦኦ";
    this.s25 = "ልምርስሽቅብቭትችኅንኝክኽዝዥድጅግጥጭጵፍፕ";
    this.s26 = "ሉሙሩሱሹቁቡቩቱቹኁኑኙኩኹዙዡዱጁጉጡጩጱፉፑ";
    this.s27 = "ሏሟሯሷሿቋቧቯቷቿኋኗኟኳዃዟዧዷጇጓጧጯጷፏፗ";
    this.s28 = "ሙቡቁኩኹጉፉፑ";
    this.s29 = "ᎀᎄቈኰዀጐᎈᎌ";
    this.s30 = "ᎃᎇቍኵዅጕᎉᎍ";
    this.s31 = "ᎁᎅቊኲዂጒᎋᎏ";
    this.s32 = "ሟቧቋኳዃጓፏፗ";
    this.s33 = "ᎂᎆቌኴዄጔᎊᎎ";
    this.s34 = "23456789";
    this.s35 = "፪፫፬፭፮፯፰፱";
    this.s36 = "፳፴፵፶፷፸፹፺";
    this.s37 = "123456789";
    this.s38 = "፩፪፫፬፭፮፯፰፱፲፳፴፵፶፷፸፹፺፻፼";
    this.s39 = "0123456789";
    this.s40 = ":,";
    this.s41 = "፡፣";
    this.s42 = "eEäÄéÉèÈêÊëË";
    this.s43 = "uUùÙúÚûÛüÜ";
    this.s44 = "iIìÌíÍîÎïÏ";
    this.s45 = "aAàÀáÁâÂãÃåÅ";
    this.s46 = "æÆ";
    this.s47 = "oOòÒóÓôÔõÕöÖøØ";
    this.s48 = "ÇçÐðÝýÿßÑñ";
    this.s49 = "ጭችድድይይይጽኝኝ";
    this.s50 = "àÀáÁâÂãÃåÅùÙúÚûÛüÜìÌíÍîÎïÏæÆéÉèÈêÊëËòÒóÓôÔõÕöÖøØ";
    this.s51 = "አአአአአአአአአአኡኡኡኡኡኡኡኡኢኢኢኢኢኢኢኢኤኤእእእእእእእእኦኦኦኦኦኦኦኦኦኦኦኦ";
    this.gs = function(t, e) {
        return this.g0(t, e);
    };
    this.g0 = function(t, e) {
        var k = KeymanWeb,
            m = 0;
        if (k.KKM(e, 16400, 32)) {
            m = 1;
            k.KO(0, t, "፡");
        } else if (k.KKM(e, 16384, 222) && k.KA(0, k.KC(1, 1, t), this.s15)) {
            m = 1;
            k.KIO(1, this.s15, 1, t);
            k.KDO(-1, t, 0);
        } else if (k.KKM(e, 16384, 222) && k.KDM(0, t, 0)) {
            m = 1;
            k.KO(0, t, "'");
        } else if (k.KKM(e, 16384, 188) && k.KCM(1, t, "፣", 1)) {
            m = 1;
            k.KO(1, t, "፥");
        } else if (k.KKM(e, 16384, 188) && k.KCM(1, t, "፥", 1)) {
            m = 1;
            k.KO(1, t, ",");
        } else if (k.KKM(e, 16384, 188) && k.KCM(1, t, ",", 1)) {
            m = 1;
            k.KO(1, t, "፣");
        } else if (k.KKM(e, 16384, 188)) {
            m = 1;
            k.KO(0, t, "፣");
        } else if (k.KKM(e, 16384, 189) && k.KCM(1, t, "፡", 1)) {
            m = 1;
            k.KO(1, t, "፦");
        } else if (k.KKM(e, 16384, 48) && k.KCM(3, t, "፲፻፼", 3)) {
            m = 1;
            k.KO(3, t, "፼፼");
        } else if (k.KKM(e, 16384, 48) && k.KA(0, k.KC(3, 1, t), this.s35) && k.KCM(2, t, "፻ ", 2)) {
            m = 1;
            k.KIO(3, this.s36, 1, t);
            k.KO(-1, t, "፻");
        } else if (k.KKM(e, 16384, 48) && k.KA(0, k.KC(3, 1, t), this.s35) && k.KCM(2, t, "፻፼", 2)) {
            m = 1;
            k.KIO(3, this.s36, 1, t);
            k.KO(-1, t, "፻፼");
        } else if (k.KKM(e, 16384, 48) && k.KA(0, k.KC(3, 1, t), this.s36) && k.KCM(2, t, "፻፼", 2)) {
            m = 1;
            k.KIO(3, this.s35, 1, t);
            k.KO(-1, t, "፼፼");
        } else if (k.KKM(e, 16384, 48) && k.KCM(2, t, "፲፻", 2)) {
            m = 1;
            k.KO(2, t, "፼");
        } else if (k.KKM(e, 16384, 48) && k.KCM(2, t, "፲፼", 2)) {
            m = 1;
            k.KO(2, t, "፻፼");
        } else if (k.KKM(e, 16384, 48) && k.KCM(2, t, "፻፼", 2)) {
            m = 1;
            k.KO(2, t, "፲፻፼");
        } else if (k.KKM(e, 16384, 48) && k.KCM(2, t, "፼፼", 2)) {
            m = 1;
            k.KB(t);
        } else if (k.KKM(e, 16384, 48) && k.KA(0, k.KC(2, 1, t), this.s36) && k.KCM(1, t, "፻", 1)) {
            m = 1;
            k.KIO(2, this.s35, 1, t);
            k.KO(-1, t, "፼");
        } else if (k.KKM(e, 16384, 48) && k.KA(0, k.KC(2, 1, t), this.s35) && k.KCM(1, t, "፼", 1)) {
            m = 1;
            k.KIO(2, this.s36, 1, t);
            k.KO(-1, t, "፼");
        } else if (k.KKM(e, 16384, 48) && k.KA(0, k.KC(2, 1, t), this.s36) && k.KCM(1, t, "፼", 1)) {
            m = 1;
            k.KIO(2, this.s35, 1, t);
            k.KO(-1, t, "፻፼");
        } else if (k.KKM(e, 16384, 48) && k.KCM(1, t, "፩", 1)) {
            m = 1;
            k.KO(1, t, "፲");
        } else if (k.KKM(e, 16384, 48) && k.KCM(1, t, "፲", 1)) {
            m = 1;
            k.KO(1, t, "፻");
        } else if (k.KKM(e, 16384, 48) && k.KCM(1, t, "፻", 1)) {
            m = 1;
            k.KO(1, t, "፲፻");
        } else if (k.KKM(e, 16384, 48) && k.KCM(1, t, "፼", 1)) {
            m = 1;
            k.KO(1, t, "፲፼");
        } else if (k.KKM(e, 16384, 48) && k.KA(0, k.KC(1, 1, t), this.s35)) {
            m = 1;
            k.KIO(1, this.s36, 1, t);
        } else if (k.KKM(e, 16384, 48) && k.KA(0, k.KC(1, 1, t), this.s36)) {
            m = 1;
            k.KIO(1, this.s35, 1, t);
            k.KO(-1, t, "፻");
        } else if (k.KKM(e, 16384, 49) && k.KA(0, k.KC(2, 1, t), this.s39) && k.KA(1, k.KC(1, 1, t), this.s41)) {
            m = 1;
            k.KIO(2, this.s39, 1, t);
            k.KIO(-1, this.s40, 2, t);
            k.KO(-1, t, "1");
        } else if (k.KKM(e, 16384, 49) && k.KCM(1, t, "'", 1)) {
            m = 1;
            k.KO(1, t, "፩");
        } else if (k.KKM(e, 16384, 49) && k.KA(0, k.KC(1, 1, t), this.s38)) {
            m = 1;
            k.KIO(1, this.s38, 1, t);
            k.KO(-1, t, "፩");
        } else if (k.KKM(e, 16384, 50) && k.KA(0, k.KC(2, 1, t), this.s39) && k.KA(1, k.KC(1, 1, t), this.s41)) {
            m = 1;
            k.KIO(2, this.s39, 1, t);
            k.KIO(-1, this.s40, 2, t);
            k.KO(-1, t, "2");
        } else if (k.KKM(e, 16384, 50) && k.KCM(1, t, "'", 1)) {
            m = 1;
            k.KO(1, t, "፪");
        } else if (k.KKM(e, 16384, 50) && k.KA(0, k.KC(1, 1, t), this.s38)) {
            m = 1;
            k.KIO(1, this.s38, 1, t);
            k.KO(-1, t, "፪");
        } else if (k.KKM(e, 16384, 51) && k.KA(0, k.KC(2, 1, t), this.s39) && k.KA(1, k.KC(1, 1, t), this.s41)) {
            m = 1;
            k.KIO(2, this.s39, 1, t);
            k.KIO(-1, this.s40, 2, t);
            k.KO(-1, t, "3");
        } else if (k.KKM(e, 16384, 51) && k.KCM(1, t, "'", 1)) {
            m = 1;
            k.KO(1, t, "፫");
        } else if (k.KKM(e, 16384, 51) && k.KA(0, k.KC(1, 1, t), this.s38)) {
            m = 1;
            k.KIO(1, this.s38, 1, t);
            k.KO(-1, t, "፫");
        } else if (k.KKM(e, 16384, 52) && k.KA(0, k.KC(2, 1, t), this.s39) && k.KA(1, k.KC(1, 1, t), this.s41)) {
            m = 1;
            k.KIO(2, this.s39, 1, t);
            k.KIO(-1, this.s40, 2, t);
            k.KO(-1, t, "4");
        } else if (k.KKM(e, 16384, 52) && k.KCM(1, t, "'", 1)) {
            m = 1;
            k.KO(1, t, "፬");
        } else if (k.KKM(e, 16384, 52) && k.KA(0, k.KC(1, 1, t), this.s38)) {
            m = 1;
            k.KIO(1, this.s38, 1, t);
            k.KO(-1, t, "፬");
        } else if (k.KKM(e, 16384, 53) && k.KA(0, k.KC(2, 1, t), this.s39) && k.KA(1, k.KC(1, 1, t), this.s41)) {
            m = 1;
            k.KIO(2, this.s39, 1, t);
            k.KIO(-1, this.s40, 2, t);
            k.KO(-1, t, "5");
        } else if (k.KKM(e, 16384, 53) && k.KCM(1, t, "'", 1)) {
            m = 1;
            k.KO(1, t, "፭");
        } else if (k.KKM(e, 16384, 53) && k.KA(0, k.KC(1, 1, t), this.s38)) {
            m = 1;
            k.KIO(1, this.s38, 1, t);
            k.KO(-1, t, "፭");
        } else if (k.KKM(e, 16384, 54) && k.KA(0, k.KC(2, 1, t), this.s39) && k.KA(1, k.KC(1, 1, t), this.s41)) {
            m = 1;
            k.KIO(2, this.s39, 1, t);
            k.KIO(-1, this.s40, 2, t);
            k.KO(-1, t, "6");
        } else if (k.KKM(e, 16384, 54) && k.KCM(1, t, "'", 1)) {
            m = 1;
            k.KO(1, t, "፮");
        } else if (k.KKM(e, 16384, 54) && k.KA(0, k.KC(1, 1, t), this.s38)) {
            m = 1;
            k.KIO(1, this.s38, 1, t);
            k.KO(-1, t, "፮");
        } else if (k.KKM(e, 16384, 55) && k.KA(0, k.KC(2, 1, t), this.s39) && k.KA(1, k.KC(1, 1, t), this.s41)) {
            m = 1;
            k.KIO(2, this.s39, 1, t);
            k.KIO(-1, this.s40, 2, t);
            k.KO(-1, t, "7");
        } else if (k.KKM(e, 16384, 55) && k.KCM(1, t, "'", 1)) {
            m = 1;
            k.KO(1, t, "፯");
        } else if (k.KKM(e, 16384, 55) && k.KA(0, k.KC(1, 1, t), this.s38)) {
            m = 1;
            k.KIO(1, this.s38, 1, t);
            k.KO(-1, t, "፯");
        } else if (k.KKM(e, 16384, 56) && k.KA(0, k.KC(2, 1, t), this.s39) && k.KA(1, k.KC(1, 1, t), this.s41)) {
            m = 1;
            k.KIO(2, this.s39, 1, t);
            k.KIO(-1, this.s40, 2, t);
            k.KO(-1, t, "8");
        } else if (k.KKM(e, 16384, 56) && k.KCM(1, t, "'", 1)) {
            m = 1;
            k.KO(1, t, "፰");
        } else if (k.KKM(e, 16384, 56) && k.KA(0, k.KC(1, 1, t), this.s38)) {
            m = 1;
            k.KIO(1, this.s38, 1, t);
            k.KO(-1, t, "፰");
        } else if (k.KKM(e, 16384, 57) && k.KA(0, k.KC(2, 1, t), this.s39) && k.KA(1, k.KC(1, 1, t), this.s41)) {
            m = 1;
            k.KIO(2, this.s39, 1, t);
            k.KIO(-1, this.s40, 2, t);
            k.KO(-1, t, "9");
        } else if (k.KKM(e, 16384, 57) && k.KCM(1, t, "'", 1)) {
            m = 1;
            k.KO(1, t, "፱");
        } else if (k.KKM(e, 16384, 57) && k.KA(0, k.KC(1, 1, t), this.s38)) {
            m = 1;
            k.KIO(1, this.s38, 1, t);
            k.KO(-1, t, "፱");
        } else if (k.KKM(e, 16400, 186) && k.KCM(1, t, "፡", 1)) {
            m = 1;
            k.KO(1, t, "።");
        } else if (k.KKM(e, 16400, 186) && k.KCM(1, t, "።", 1)) {
            m = 1;
            k.KO(1, t, ":");
        } else if (k.KKM(e, 16400, 186) && k.KCM(1, t, ":", 1)) {
            m = 1;
            k.KO(1, t, "፡");
        } else if (k.KKM(e, 16400, 186)) {
            m = 1;
            k.KO(0, t, "፡");
        } else if (k.KKM(e, 16384, 186) && k.KCM(1, t, "፤", 1)) {
            m = 1;
            k.KO(1, t, ";");
        } else if (k.KKM(e, 16384, 186) && k.KCM(1, t, ";", 1)) {
            m = 1;
            k.KO(1, t, "፤");
        } else if (k.KKM(e, 16384, 186)) {
            m = 1;
            k.KO(0, t, "፤");
        } else if (k.KKM(e, 16400, 188) && k.KCM(1, t, "‹", 1)) {
            m = 1;
            k.KO(1, t, "«");
        } else if (k.KKM(e, 16400, 188) && k.KCM(1, t, "«", 1)) {
            m = 1;
            k.KO(1, t, "<");
        } else if (k.KKM(e, 16400, 188)) {
            m = 1;
            k.KO(0, t, "‹");
        } else if (k.KKM(e, 16400, 190) && k.KCM(1, t, "›", 1)) {
            m = 1;
            k.KO(1, t, "»");
        } else if (k.KKM(e, 16400, 190) && k.KCM(1, t, "»", 1)) {
            m = 1;
            k.KO(1, t, ">");
        } else if (k.KKM(e, 16400, 190)) {
            m = 1;
            k.KO(0, t, "›");
        } else if (k.KKM(e, 16400, 65) && k.KCM(1, t, "እ", 1)) {
            m = 1;
            k.KO(1, t, "አ");
        } else if (k.KKM(e, 16400, 65) && k.KA(0, k.KC(1, 1, t), this.s15)) {
            m = 1;
            k.KIO(1, this.s13, 1, t);
        } else if (k.KKM(e, 16400, 65) && k.KA(0, k.KC(1, 1, t), this.s26)) {
            m = 1;
            k.KIO(1, this.s27, 1, t);
        } else if (k.KKM(e, 16400, 65)) {
            m = 1;
            k.KO(0, t, "አ");
        } else if (k.KKM(e, 16400, 66)) {
            m = 1;
            k.KO(0, t, "ብ");
        } else if (k.KKM(e, 16400, 67)) {
            m = 1;
            k.KO(0, t, "ጭ");
        } else if (k.KKM(e, 16400, 68)) {
            m = 1;
            k.KO(0, t, "ድ");
        } else if (k.KKM(e, 16400, 69) && k.KCM(1, t, "አ", 1)) {
            m = 1;
            k.KO(1, t, "ኧ");
        } else if (k.KKM(e, 16400, 69) && k.KCM(1, t, "እ", 1)) {
            m = 1;
            k.KO(1, t, "ኧ");
        } else if (k.KKM(e, 16400, 69) && k.KCM(1, t, "ኢ", 1)) {
            m = 1;
            k.KO(1, t, "ኤ");
        } else if (k.KKM(e, 16400, 69) && k.KA(0, k.KC(1, 1, t), this.s15)) {
            m = 1;
            k.KIO(1, this.s10, 1, t);
        } else if (k.KKM(e, 16400, 69) && k.KA(0, k.KC(1, 1, t), this.s12)) {
            m = 1;
            k.KIO(1, this.s14, 1, t);
        } else if (k.KKM(e, 16400, 69) && k.KA(0, k.KC(1, 1, t), this.s28)) {
            m = 1;
            k.KIO(1, this.s29, 1, t);
        } else if (k.KKM(e, 16400, 69) && k.KA(0, k.KC(1, 1, t), this.s31)) {
            m = 1;
            k.KIO(1, this.s33, 1, t);
        } else if (k.KKM(e, 16400, 69)) {
            m = 1;
            k.KO(0, t, "እ");
        } else if (k.KKM(e, 16400, 70)) {
            m = 1;
            k.KO(0, t, "ፍ");
        } else if (k.KKM(e, 16400, 71)) {
            m = 1;
            k.KO(0, t, "ግ");
        } else if (k.KKM(e, 16400, 72)) {
            m = 1;
            k.KO(0, t, "ኽ");
        } else if (k.KKM(e, 16400, 73) && k.KCM(1, t, "እ", 1)) {
            m = 1;
            k.KO(1, t, "ኢ");
        } else if (k.KKM(e, 16400, 73) && k.KA(0, k.KC(1, 1, t), this.s15)) {
            m = 1;
            k.KIO(1, this.s12, 1, t);
        } else if (k.KKM(e, 16400, 73) && k.KA(0, k.KC(1, 1, t), this.s28)) {
            m = 1;
            k.KIO(1, this.s31, 1, t);
        } else if (k.KKM(e, 16400, 73)) {
            m = 1;
            k.KO(0, t, "ኢ");
        } else if (k.KKM(e, 16400, 74)) {
            m = 1;
            k.KO(0, t, "ጅ");
        } else if (k.KKM(e, 16400, 75)) {
            m = 1;
            k.KO(0, t, "ክ");
        } else if (k.KKM(e, 16400, 76)) {
            m = 1;
            k.KO(0, t, "ል");
        } else if (k.KKM(e, 16400, 77)) {
            m = 1;
            k.KO(0, t, "ም");
        } else if (k.KKM(e, 16400, 78)) {
            m = 1;
            k.KO(0, t, "ኝ");
        } else if (k.KKM(e, 16400, 79) && k.KCM(1, t, "እ", 1)) {
            m = 1;
            k.KO(1, t, "ኦ");
        } else if (k.KKM(e, 16400, 79) && k.KA(0, k.KC(1, 1, t), this.s15)) {
            m = 1;
            k.KIO(1, this.s16, 1, t);
        } else if (k.KKM(e, 16400, 79)) {
            m = 1;
            k.KO(0, t, "ኦ");
        } else if (k.KKM(e, 16400, 80)) {
            m = 1;
            k.KO(0, t, "ጵ");
        } else if (k.KKM(e, 16400, 81)) {
            m = 1;
            k.KO(0, t, "ቅ");
        } else if (k.KKM(e, 16400, 82)) {
            m = 1;
            k.KO(0, t, "ር");
        } else if (k.KKM(e, 16400, 83)) {
            m = 1;
            k.KO(0, t, "ፅ");
        } else if (k.KKM(e, 16400, 84)) {
            m = 1;
            k.KO(0, t, "ጥ");
        } else if (k.KKM(e, 16400, 85) && k.KCM(1, t, "እ", 1)) {
            m = 1;
            k.KO(1, t, "ኡ");
        } else if (k.KKM(e, 16400, 85) && k.KA(0, k.KC(1, 1, t), this.s15)) {
            m = 1;
            k.KIO(1, this.s11, 1, t);
        } else if (k.KKM(e, 16400, 85) && k.KA(0, k.KC(1, 1, t), this.s28)) {
            m = 1;
            k.KIO(1, this.s30, 1, t);
        } else if (k.KKM(e, 16400, 85)) {
            m = 1;
            k.KO(0, t, "ኡ");
        } else if (k.KKM(e, 16400, 86)) {
            m = 1;
            k.KO(0, t, "ብ");
        } else if (k.KKM(e, 16400, 87)) {
            m = 1;
            k.KO(0, t, "ው");
        } else if (k.KKM(e, 16400, 88)) {
            m = 1;
            k.KO(0, t, "ሽ");
        } else if (k.KKM(e, 16400, 89) && k.KA(0, k.KC(1, 1, t), this.s20)) {
            m = 1;
            k.KIO(1, this.s21, 1, t);
        } else if (k.KKM(e, 16400, 89)) {
            m = 1;
            k.KO(0, t, "ይ");
        } else if (k.KKM(e, 16400, 90)) {
            m = 1;
            k.KO(0, t, "ዥ");
        } else if (k.KKM(e, 16400, 189) && k.KCM(1, t, "፟", 1)) {
            m = 1;
            k.KO(1, t, "_");
        } else if (k.KKM(e, 16400, 189)) {
            m = 1;
            k.KO(0, t, "፟");
        } else if (k.KKM(e, 16400, 189)) {
            m = 1;
            k.KO(0, t, "ⷕ");
        } else if (k.KKM(e, 16400, 189)) {
            m = 1;
            k.KO(0, t, "ⷝ");
        } else if (k.KKM(e, 16400, 189)) {
            m = 1;
            k.KO(0, t, "ⷅ");
        } else if (k.KKM(e, 16400, 189)) {
            m = 1;
            k.KO(0, t, "ⷍ");
        } else if (k.KKM(e, 16384, 65) && k.KCM(1, t, "እ", 1)) {
            m = 1;
            k.KO(1, t, "አ");
        } else if (k.KKM(e, 16384, 65) && k.KA(0, k.KC(1, 1, t), this.s15)) {
            m = 1;
            k.KIO(1, this.s13, 1, t);
        } else if (k.KKM(e, 16384, 65) && k.KA(0, k.KC(1, 1, t), this.s26)) {
            m = 1;
            k.KIO(1, this.s27, 1, t);
        } else if (k.KKM(e, 16384, 65)) {
            m = 1;
            k.KO(0, t, "አ");
        } else if (k.KKM(e, 16384, 66)) {
            m = 1;
            k.KO(0, t, "ብ");
        } else if (k.KKM(e, 16384, 67)) {
            m = 1;
            k.KO(0, t, "ች");
        } else if (k.KKM(e, 16384, 68)) {
            m = 1;
            k.KO(0, t, "ድ");
        } else if (k.KKM(e, 16384, 69) && k.KCM(1, t, "አ", 1)) {
            m = 1;
            k.KO(1, t, "ኧ");
        } else if (k.KKM(e, 16384, 69) && k.KCM(1, t, "እ", 1)) {
            m = 1;
            k.KO(1, t, "ኧ");
        } else if (k.KKM(e, 16384, 69) && k.KCM(1, t, "ኢ", 1)) {
            m = 1;
            k.KO(1, t, "ኤ");
        } else if (k.KKM(e, 16384, 69) && k.KA(0, k.KC(1, 1, t), this.s15)) {
            m = 1;
            k.KIO(1, this.s10, 1, t);
        } else if (k.KKM(e, 16384, 69) && k.KA(0, k.KC(1, 1, t), this.s12)) {
            m = 1;
            k.KIO(1, this.s14, 1, t);
        } else if (k.KKM(e, 16384, 69) && k.KA(0, k.KC(1, 1, t), this.s28)) {
            m = 1;
            k.KIO(1, this.s29, 1, t);
        } else if (k.KKM(e, 16384, 69) && k.KA(0, k.KC(1, 1, t), this.s31)) {
            m = 1;
            k.KIO(1, this.s33, 1, t);
        } else if (k.KKM(e, 16384, 69)) {
            m = 1;
            k.KO(0, t, "እ");
        } else if (k.KKM(e, 16384, 70)) {
            m = 1;
            k.KO(0, t, "ፍ");
        } else if (k.KKM(e, 16384, 71)) {
            m = 1;
            k.KO(0, t, "ግ");
        } else if (k.KKM(e, 16384, 72)) {
            m = 1;
            k.KO(0, t, "ኽ");
        } else if (k.KKM(e, 16384, 73) && k.KCM(1, t, "እ", 1)) {
            m = 1;
            k.KO(1, t, "ኢ");
        } else if (k.KKM(e, 16384, 73) && k.KA(0, k.KC(1, 1, t), this.s15)) {
            m = 1;
            k.KIO(1, this.s12, 1, t);
        } else if (k.KKM(e, 16384, 73) && k.KA(0, k.KC(1, 1, t), this.s28)) {
            m = 1;
            k.KIO(1, this.s31, 1, t);
        } else if (k.KKM(e, 16384, 73)) {
            m = 1;
            k.KO(0, t, "ኢ");
        } else if (k.KKM(e, 16384, 74)) {
            m = 1;
            k.KO(0, t, "ጅ");
        } else if (k.KKM(e, 16384, 75)) {
            m = 1;
            k.KO(0, t, "ክ");
        } else if (k.KKM(e, 16384, 76)) {
            m = 1;
            k.KO(0, t, "ል");
        } else if (k.KKM(e, 16384, 77)) {
            m = 1;
            k.KO(0, t, "ም");
        } else if (k.KKM(e, 16384, 78)) {
            m = 1;
            k.KO(0, t, "ን");
        } else if (k.KKM(e, 16384, 79) && k.KCM(1, t, "እ", 1)) {
            m = 1;
            k.KO(1, t, "ኦ");
        } else if (k.KKM(e, 16384, 79) && k.KA(0, k.KC(1, 1, t), this.s15)) {
            m = 1;
            k.KIO(1, this.s16, 1, t);
        } else if (k.KKM(e, 16384, 79) && k.KCM(1, t, "ኮ", 1)) {
            m = 1;
            k.KO(1, t, "ኰ");
        } else if (k.KKM(e, 16384, 79) && k.KCM(1, t, "ጎ", 1)) {
            m = 1;
            k.KO(1, t, "ጐ");
        } else if (k.KKM(e, 16384, 79) && k.KCM(1, t, "ኰ", 1)) {
            m = 1;
            k.KO(1, t, "ኮ");
        } else if (k.KKM(e, 16384, 79) && k.KCM(1, t, "ጐ", 1)) {
            m = 1;
            k.KO(1, t, "ጎ");
        } else if (k.KKM(e, 16384, 79)) {
            m = 1;
            k.KO(0, t, "ኦ");
        } else if (k.KKM(e, 16384, 80)) {
            m = 1;
            k.KO(0, t, "ፕ");
        } else if (k.KKM(e, 16384, 81)) {
            m = 1;
            k.KO(0, t, "ቅ");
        } else if (k.KKM(e, 16384, 82)) {
            m = 1;
            k.KO(0, t, "ር");
        } else if (k.KKM(e, 16384, 83)) {
            m = 1;
            k.KO(0, t, "ስ");
        } else if (k.KKM(e, 16384, 84)) {
            m = 1;
            k.KO(0, t, "ት");
        } else if (k.KKM(e, 16384, 85) && k.KCM(1, t, "እ", 1)) {
            m = 1;
            k.KO(1, t, "ኡ");
        } else if (k.KKM(e, 16384, 85) && k.KA(0, k.KC(1, 1, t), this.s15)) {
            m = 1;
            k.KIO(1, this.s11, 1, t);
        } else if (k.KKM(e, 16384, 85) && k.KA(0, k.KC(1, 1, t), this.s28)) {
            m = 1;
            k.KIO(1, this.s30, 1, t);
        } else if (k.KKM(e, 16384, 85)) {
            m = 1;
            k.KO(0, t, "ኡ");
        } else if (k.KKM(e, 16384, 86)) {
            m = 1;
            k.KO(0, t, "ብ");
        } else if (k.KKM(e, 16384, 87)) {
            m = 1;
            k.KO(0, t, "ው");
        } else if (k.KKM(e, 16384, 88)) {
            m = 1;
            k.KO(0, t, "ሽ");
        } else if (k.KKM(e, 16384, 89) && k.KA(0, k.KC(1, 1, t), this.s20)) {
            m = 1;
            k.KIO(1, this.s21, 1, t);
        } else if (k.KKM(e, 16384, 89)) {
            m = 1;
            k.KO(0, t, "ይ");
        } else if (k.KKM(e, 16384, 90)) {
            m = 1;
            k.KO(0, t, "ዝ");
        } else if (k.KKM(e, 16384, 0)) {
            m = 1;
            k.KO(0, t, "፟");
        } else if (k.KKM(e, 16384, 0) && k.KCM(1, t, "እ", 1)) {
            m = 1;
            k.KO(1, t, "አ");
        } else if (k.KKM(e, 16384, 0) && k.KA(0, k.KC(1, 1, t), this.s15)) {
            m = 1;
            k.KIO(1, this.s13, 1, t);
        } else if (k.KKM(e, 16384, 0) && k.KA(0, k.KC(1, 1, t), this.s26)) {
            m = 1;
            k.KIO(1, this.s27, 1, t);
        } else if (k.KKM(e, 16384, 0)) {
            m = 1;
            k.KO(0, t, "አ");
        } else if (k.KKM(e, 16384, 0) && k.KCM(1, t, "እ", 1)) {
            m = 1;
            k.KO(1, t, "አ");
        } else if (k.KKM(e, 16384, 0) && k.KA(0, k.KC(1, 1, t), this.s15)) {
            m = 1;
            k.KIO(1, this.s13, 1, t);
        } else if (k.KKM(e, 16384, 0) && k.KA(0, k.KC(1, 1, t), this.s26)) {
            m = 1;
            k.KIO(1, this.s27, 1, t);
        } else if (k.KKM(e, 16384, 0)) {
            m = 1;
            k.KO(0, t, "አ");
        } else if (k.KKM(e, 16384, 0) && k.KCM(1, t, "እ", 1)) {
            m = 1;
            k.KO(1, t, "አ");
        } else if (k.KKM(e, 16384, 0) && k.KA(0, k.KC(1, 1, t), this.s15)) {
            m = 1;
            k.KIO(1, this.s13, 1, t);
        } else if (k.KKM(e, 16384, 0) && k.KA(0, k.KC(1, 1, t), this.s26)) {
            m = 1;
            k.KIO(1, this.s27, 1, t);
        } else if (k.KKM(e, 16384, 0)) {
            m = 1;
            k.KO(0, t, "አ");
        } else if (k.KKM(e, 16384, 0) && k.KCM(1, t, "እ", 1)) {
            m = 1;
            k.KO(1, t, "አ");
        } else if (k.KKM(e, 16384, 0) && k.KA(0, k.KC(1, 1, t), this.s15)) {
            m = 1;
            k.KIO(1, this.s13, 1, t);
        } else if (k.KKM(e, 16384, 0) && k.KA(0, k.KC(1, 1, t), this.s26)) {
            m = 1;
            k.KIO(1, this.s27, 1, t);
        } else if (k.KKM(e, 16384, 0)) {
            m = 1;
            k.KO(0, t, "አ");
        } else if (k.KKM(e, 16384, 0) && k.KCM(1, t, "አ", 1)) {
            m = 1;
            k.KO(1, t, "ኧ");
        } else if (k.KKM(e, 16384, 0) && k.KCM(1, t, "እ", 1)) {
            m = 1;
            k.KO(1, t, "ኧ");
        } else if (k.KKM(e, 16384, 0) && k.KCM(1, t, "ኢ", 1)) {
            m = 1;
            k.KO(1, t, "ኤ");
        } else if (k.KKM(e, 16384, 0) && k.KA(0, k.KC(1, 1, t), this.s15)) {
            m = 1;
            k.KIO(1, this.s10, 1, t);
        } else if (k.KKM(e, 16384, 0) && k.KA(0, k.KC(1, 1, t), this.s12)) {
            m = 1;
            k.KIO(1, this.s14, 1, t);
        } else if (k.KKM(e, 16384, 0) && k.KA(0, k.KC(1, 1, t), this.s28)) {
            m = 1;
            k.KIO(1, this.s29, 1, t);
        } else if (k.KKM(e, 16384, 0) && k.KA(0, k.KC(1, 1, t), this.s31)) {
            m = 1;
            k.KIO(1, this.s33, 1, t);
        } else if (k.KKM(e, 16384, 0) && k.KCM(1, t, "እ", 1)) {
            m = 1;
            k.KO(1, t, "አ");
        } else if (k.KKM(e, 16384, 0) && k.KA(0, k.KC(1, 1, t), this.s15)) {
            m = 1;
            k.KIO(1, this.s13, 1, t);
        } else if (k.KKM(e, 16384, 0) && k.KA(0, k.KC(1, 1, t), this.s26)) {
            m = 1;
            k.KIO(1, this.s27, 1, t);
        } else if (k.KKM(e, 16384, 0)) {
            m = 1;
            k.KO(0, t, "አ");
        } else if (k.KKM(e, 16384, 0) && k.KA(0, k.KC(1, 1, t), this.s15)) {
            m = 1;
            k.KIO(1, this.s14, 1, t);
        } else if (k.KKM(e, 16384, 0)) {
            m = 1;
            k.KO(0, t, "ኤ");
        } else if (k.KKM(e, 16384, 0)) {
            m = 1;
            k.KO(0, t, "ጭ");
        } else if (k.KKM(e, 16384, 0) && k.KCM(1, t, "አ", 1)) {
            m = 1;
            k.KO(1, t, "ኧ");
        } else if (k.KKM(e, 16384, 0) && k.KCM(1, t, "እ", 1)) {
            m = 1;
            k.KO(1, t, "ኧ");
        } else if (k.KKM(e, 16384, 0) && k.KCM(1, t, "ኢ", 1)) {
            m = 1;
            k.KO(1, t, "ኤ");
        } else if (k.KKM(e, 16384, 0) && k.KA(0, k.KC(1, 1, t), this.s15)) {
            m = 1;
            k.KIO(1, this.s10, 1, t);
        } else if (k.KKM(e, 16384, 0) && k.KA(0, k.KC(1, 1, t), this.s12)) {
            m = 1;
            k.KIO(1, this.s14, 1, t);
        } else if (k.KKM(e, 16384, 0) && k.KA(0, k.KC(1, 1, t), this.s28)) {
            m = 1;
            k.KIO(1, this.s29, 1, t);
        } else if (k.KKM(e, 16384, 0) && k.KA(0, k.KC(1, 1, t), this.s31)) {
            m = 1;
            k.KIO(1, this.s33, 1, t);
        } else if (k.KKM(e, 16384, 0)) {
            m = 1;
            k.KO(0, t, "እ");
        } else if (k.KKM(e, 16384, 0) && k.KCM(1, t, "አ", 1)) {
            m = 1;
            k.KO(1, t, "ኧ");
        } else if (k.KKM(e, 16384, 0) && k.KCM(1, t, "እ", 1)) {
            m = 1;
            k.KO(1, t, "ኧ");
        } else if (k.KKM(e, 16384, 0) && k.KCM(1, t, "ኢ", 1)) {
            m = 1;
            k.KO(1, t, "ኤ");
        } else if (k.KKM(e, 16384, 0) && k.KA(0, k.KC(1, 1, t), this.s15)) {
            m = 1;
            k.KIO(1, this.s10, 1, t);
        } else if (k.KKM(e, 16384, 0) && k.KA(0, k.KC(1, 1, t), this.s12)) {
            m = 1;
            k.KIO(1, this.s14, 1, t);
        } else if (k.KKM(e, 16384, 0) && k.KA(0, k.KC(1, 1, t), this.s28)) {
            m = 1;
            k.KIO(1, this.s29, 1, t);
        } else if (k.KKM(e, 16384, 0) && k.KA(0, k.KC(1, 1, t), this.s31)) {
            m = 1;
            k.KIO(1, this.s33, 1, t);
        } else if (k.KKM(e, 16384, 0)) {
            m = 1;
            k.KO(0, t, "እ");
        } else if (k.KKM(e, 16384, 0) && k.KCM(1, t, "አ", 1)) {
            m = 1;
            k.KO(1, t, "ኧ");
        } else if (k.KKM(e, 16384, 0) && k.KCM(1, t, "እ", 1)) {
            m = 1;
            k.KO(1, t, "ኧ");
        } else if (k.KKM(e, 16384, 0) && k.KCM(1, t, "ኢ", 1)) {
            m = 1;
            k.KO(1, t, "ኤ");
        } else if (k.KKM(e, 16384, 0) && k.KA(0, k.KC(1, 1, t), this.s15)) {
            m = 1;
            k.KIO(1, this.s10, 1, t);
        } else if (k.KKM(e, 16384, 0) && k.KA(0, k.KC(1, 1, t), this.s12)) {
            m = 1;
            k.KIO(1, this.s14, 1, t);
        } else if (k.KKM(e, 16384, 0) && k.KA(0, k.KC(1, 1, t), this.s28)) {
            m = 1;
            k.KIO(1, this.s29, 1, t);
        } else if (k.KKM(e, 16384, 0) && k.KA(0, k.KC(1, 1, t), this.s31)) {
            m = 1;
            k.KIO(1, this.s33, 1, t);
        } else if (k.KKM(e, 16384, 0)) {
            m = 1;
            k.KO(0, t, "እ");
        } else if (k.KKM(e, 16384, 0) && k.KCM(1, t, "አ", 1)) {
            m = 1;
            k.KO(1, t, "ኧ");
        } else if (k.KKM(e, 16384, 0) && k.KCM(1, t, "እ", 1)) {
            m = 1;
            k.KO(1, t, "ኧ");
        } else if (k.KKM(e, 16384, 0) && k.KCM(1, t, "ኢ", 1)) {
            m = 1;
            k.KO(1, t, "ኤ");
        } else if (k.KKM(e, 16384, 0) && k.KA(0, k.KC(1, 1, t), this.s15)) {
            m = 1;
            k.KIO(1, this.s10, 1, t);
        } else if (k.KKM(e, 16384, 0) && k.KA(0, k.KC(1, 1, t), this.s12)) {
            m = 1;
            k.KIO(1, this.s14, 1, t);
        } else if (k.KKM(e, 16384, 0) && k.KA(0, k.KC(1, 1, t), this.s28)) {
            m = 1;
            k.KIO(1, this.s29, 1, t);
        } else if (k.KKM(e, 16384, 0) && k.KA(0, k.KC(1, 1, t), this.s31)) {
            m = 1;
            k.KIO(1, this.s33, 1, t);
        } else if (k.KKM(e, 16384, 0)) {
            m = 1;
            k.KO(0, t, "እ");
        } else if (k.KKM(e, 16384, 0) && k.KCM(1, t, "እ", 1)) {
            m = 1;
            k.KO(1, t, "ኢ");
        } else if (k.KKM(e, 16384, 0) && k.KA(0, k.KC(1, 1, t), this.s15)) {
            m = 1;
            k.KIO(1, this.s12, 1, t);
        } else if (k.KKM(e, 16384, 0) && k.KA(0, k.KC(1, 1, t), this.s28)) {
            m = 1;
            k.KIO(1, this.s31, 1, t);
        } else if (k.KKM(e, 16384, 0)) {
            m = 1;
            k.KO(0, t, "ኢ");
        } else if (k.KKM(e, 16384, 0) && k.KCM(1, t, "እ", 1)) {
            m = 1;
            k.KO(1, t, "ኢ");
        } else if (k.KKM(e, 16384, 0) && k.KA(0, k.KC(1, 1, t), this.s15)) {
            m = 1;
            k.KIO(1, this.s12, 1, t);
        } else if (k.KKM(e, 16384, 0) && k.KA(0, k.KC(1, 1, t), this.s28)) {
            m = 1;
            k.KIO(1, this.s31, 1, t);
        } else if (k.KKM(e, 16384, 0)) {
            m = 1;
            k.KO(0, t, "ኢ");
        } else if (k.KKM(e, 16384, 0) && k.KCM(1, t, "እ", 1)) {
            m = 1;
            k.KO(1, t, "ኢ");
        } else if (k.KKM(e, 16384, 0) && k.KA(0, k.KC(1, 1, t), this.s15)) {
            m = 1;
            k.KIO(1, this.s12, 1, t);
        } else if (k.KKM(e, 16384, 0) && k.KA(0, k.KC(1, 1, t), this.s28)) {
            m = 1;
            k.KIO(1, this.s31, 1, t);
        } else if (k.KKM(e, 16384, 0)) {
            m = 1;
            k.KO(0, t, "ኢ");
        } else if (k.KKM(e, 16384, 0) && k.KCM(1, t, "እ", 1)) {
            m = 1;
            k.KO(1, t, "ኢ");
        } else if (k.KKM(e, 16384, 0) && k.KA(0, k.KC(1, 1, t), this.s15)) {
            m = 1;
            k.KIO(1, this.s12, 1, t);
        } else if (k.KKM(e, 16384, 0) && k.KA(0, k.KC(1, 1, t), this.s28)) {
            m = 1;
            k.KIO(1, this.s31, 1, t);
        } else if (k.KKM(e, 16384, 0)) {
            m = 1;
            k.KO(0, t, "ኢ");
        } else if (k.KKM(e, 16384, 0)) {
            m = 1;
            k.KO(0, t, "ድ");
        } else if (k.KKM(e, 16384, 0)) {
            m = 1;
            k.KO(0, t, "ኝ");
        } else if (k.KKM(e, 16384, 0) && k.KCM(1, t, "እ", 1)) {
            m = 1;
            k.KO(1, t, "ኦ");
        } else if (k.KKM(e, 16384, 0) && k.KA(0, k.KC(1, 1, t), this.s15)) {
            m = 1;
            k.KIO(1, this.s16, 1, t);
        } else if (k.KKM(e, 16384, 0)) {
            m = 1;
            k.KO(0, t, "ኦ");
        } else if (k.KKM(e, 16384, 0) && k.KCM(1, t, "እ", 1)) {
            m = 1;
            k.KO(1, t, "ኦ");
        } else if (k.KKM(e, 16384, 0) && k.KA(0, k.KC(1, 1, t), this.s15)) {
            m = 1;
            k.KIO(1, this.s16, 1, t);
        } else if (k.KKM(e, 16384, 0)) {
            m = 1;
            k.KO(0, t, "ኦ");
        } else if (k.KKM(e, 16384, 0) && k.KCM(1, t, "እ", 1)) {
            m = 1;
            k.KO(1, t, "ኦ");
        } else if (k.KKM(e, 16384, 0) && k.KA(0, k.KC(1, 1, t), this.s15)) {
            m = 1;
            k.KIO(1, this.s16, 1, t);
        } else if (k.KKM(e, 16384, 0)) {
            m = 1;
            k.KO(0, t, "ኦ");
        } else if (k.KKM(e, 16384, 0) && k.KCM(1, t, "እ", 1)) {
            m = 1;
            k.KO(1, t, "ኦ");
        } else if (k.KKM(e, 16384, 0) && k.KA(0, k.KC(1, 1, t), this.s15)) {
            m = 1;
            k.KIO(1, this.s16, 1, t);
        } else if (k.KKM(e, 16384, 0)) {
            m = 1;
            k.KO(0, t, "ኦ");
        } else if (k.KKM(e, 16384, 0) && k.KCM(1, t, "እ", 1)) {
            m = 1;
            k.KO(1, t, "ኦ");
        } else if (k.KKM(e, 16384, 0) && k.KA(0, k.KC(1, 1, t), this.s15)) {
            m = 1;
            k.KIO(1, this.s16, 1, t);
        } else if (k.KKM(e, 16384, 0)) {
            m = 1;
            k.KO(0, t, "ኦ");
        } else if (k.KKM(e, 16384, 0) && k.KCM(1, t, "እ", 1)) {
            m = 1;
            k.KO(1, t, "ኦ");
        } else if (k.KKM(e, 16384, 0) && k.KA(0, k.KC(1, 1, t), this.s15)) {
            m = 1;
            k.KIO(1, this.s16, 1, t);
        } else if (k.KKM(e, 16384, 0)) {
            m = 1;
            k.KO(0, t, "ኦ");
        } else if (k.KKM(e, 16384, 0) && k.KCM(1, t, "እ", 1)) {
            m = 1;
            k.KO(1, t, "ኡ");
        } else if (k.KKM(e, 16384, 0) && k.KA(0, k.KC(1, 1, t), this.s15)) {
            m = 1;
            k.KIO(1, this.s11, 1, t);
        } else if (k.KKM(e, 16384, 0) && k.KA(0, k.KC(1, 1, t), this.s28)) {
            m = 1;
            k.KIO(1, this.s30, 1, t);
        } else if (k.KKM(e, 16384, 0)) {
            m = 1;
            k.KO(0, t, "ኡ");
        } else if (k.KKM(e, 16384, 0) && k.KCM(1, t, "እ", 1)) {
            m = 1;
            k.KO(1, t, "ኡ");
        } else if (k.KKM(e, 16384, 0) && k.KA(0, k.KC(1, 1, t), this.s15)) {
            m = 1;
            k.KIO(1, this.s11, 1, t);
        } else if (k.KKM(e, 16384, 0) && k.KA(0, k.KC(1, 1, t), this.s28)) {
            m = 1;
            k.KIO(1, this.s30, 1, t);
        } else if (k.KKM(e, 16384, 0)) {
            m = 1;
            k.KO(0, t, "ኡ");
        } else if (k.KKM(e, 16384, 0) && k.KCM(1, t, "እ", 1)) {
            m = 1;
            k.KO(1, t, "ኡ");
        } else if (k.KKM(e, 16384, 0) && k.KA(0, k.KC(1, 1, t), this.s15)) {
            m = 1;
            k.KIO(1, this.s11, 1, t);
        } else if (k.KKM(e, 16384, 0) && k.KA(0, k.KC(1, 1, t), this.s28)) {
            m = 1;
            k.KIO(1, this.s30, 1, t);
        } else if (k.KKM(e, 16384, 0)) {
            m = 1;
            k.KO(0, t, "ኡ");
        } else if (k.KKM(e, 16384, 0) && k.KCM(1, t, "እ", 1)) {
            m = 1;
            k.KO(1, t, "ኡ");
        } else if (k.KKM(e, 16384, 0) && k.KA(0, k.KC(1, 1, t), this.s15)) {
            m = 1;
            k.KIO(1, this.s11, 1, t);
        } else if (k.KKM(e, 16384, 0) && k.KA(0, k.KC(1, 1, t), this.s28)) {
            m = 1;
            k.KIO(1, this.s30, 1, t);
        } else if (k.KKM(e, 16384, 0)) {
            m = 1;
            k.KO(0, t, "ኡ");
        } else if (k.KKM(e, 16384, 0)) {
            m = 1;
            k.KO(0, t, "ይ");
        } else if (k.KKM(e, 16384, 0) && k.KCM(1, t, "ጽ", 1)) {
            m = 1;
            k.KO(1, t, "ፅ");
        } else if (k.KKM(e, 16384, 0)) {
            m = 1;
            k.KO(0, t, "ጽ");
        } else if (k.KKM(e, 16384, 0) && k.KCM(1, t, "እ", 1)) {
            m = 1;
            k.KO(1, t, "አ");
        } else if (k.KKM(e, 16384, 0) && k.KA(0, k.KC(1, 1, t), this.s15)) {
            m = 1;
            k.KIO(1, this.s13, 1, t);
        } else if (k.KKM(e, 16384, 0) && k.KA(0, k.KC(1, 1, t), this.s26)) {
            m = 1;
            k.KIO(1, this.s27, 1, t);
        } else if (k.KKM(e, 16384, 0)) {
            m = 1;
            k.KO(0, t, "አ");
        } else if (k.KKM(e, 16384, 0) && k.KCM(1, t, "እ", 1)) {
            m = 1;
            k.KO(1, t, "አ");
        } else if (k.KKM(e, 16384, 0) && k.KA(0, k.KC(1, 1, t), this.s15)) {
            m = 1;
            k.KIO(1, this.s13, 1, t);
        } else if (k.KKM(e, 16384, 0) && k.KA(0, k.KC(1, 1, t), this.s26)) {
            m = 1;
            k.KIO(1, this.s27, 1, t);
        } else if (k.KKM(e, 16384, 0)) {
            m = 1;
            k.KO(0, t, "አ");
        } else if (k.KKM(e, 16384, 0) && k.KCM(1, t, "እ", 1)) {
            m = 1;
            k.KO(1, t, "አ");
        } else if (k.KKM(e, 16384, 0) && k.KA(0, k.KC(1, 1, t), this.s15)) {
            m = 1;
            k.KIO(1, this.s13, 1, t);
        } else if (k.KKM(e, 16384, 0) && k.KA(0, k.KC(1, 1, t), this.s26)) {
            m = 1;
            k.KIO(1, this.s27, 1, t);
        } else if (k.KKM(e, 16384, 0)) {
            m = 1;
            k.KO(0, t, "አ");
        } else if (k.KKM(e, 16384, 0) && k.KCM(1, t, "እ", 1)) {
            m = 1;
            k.KO(1, t, "አ");
        } else if (k.KKM(e, 16384, 0) && k.KA(0, k.KC(1, 1, t), this.s15)) {
            m = 1;
            k.KIO(1, this.s13, 1, t);
        } else if (k.KKM(e, 16384, 0) && k.KA(0, k.KC(1, 1, t), this.s26)) {
            m = 1;
            k.KIO(1, this.s27, 1, t);
        } else if (k.KKM(e, 16384, 0)) {
            m = 1;
            k.KO(0, t, "አ");
        } else if (k.KKM(e, 16384, 0) && k.KCM(1, t, "አ", 1)) {
            m = 1;
            k.KO(1, t, "ኧ");
        } else if (k.KKM(e, 16384, 0) && k.KCM(1, t, "እ", 1)) {
            m = 1;
            k.KO(1, t, "ኧ");
        } else if (k.KKM(e, 16384, 0) && k.KCM(1, t, "ኢ", 1)) {
            m = 1;
            k.KO(1, t, "ኤ");
        } else if (k.KKM(e, 16384, 0) && k.KA(0, k.KC(1, 1, t), this.s15)) {
            m = 1;
            k.KIO(1, this.s10, 1, t);
        } else if (k.KKM(e, 16384, 0) && k.KA(0, k.KC(1, 1, t), this.s12)) {
            m = 1;
            k.KIO(1, this.s14, 1, t);
        } else if (k.KKM(e, 16384, 0) && k.KA(0, k.KC(1, 1, t), this.s28)) {
            m = 1;
            k.KIO(1, this.s29, 1, t);
        } else if (k.KKM(e, 16384, 0) && k.KA(0, k.KC(1, 1, t), this.s31)) {
            m = 1;
            k.KIO(1, this.s33, 1, t);
        } else if (k.KKM(e, 16384, 0) && k.KCM(1, t, "እ", 1)) {
            m = 1;
            k.KO(1, t, "አ");
        } else if (k.KKM(e, 16384, 0) && k.KA(0, k.KC(1, 1, t), this.s15)) {
            m = 1;
            k.KIO(1, this.s13, 1, t);
        } else if (k.KKM(e, 16384, 0) && k.KA(0, k.KC(1, 1, t), this.s26)) {
            m = 1;
            k.KIO(1, this.s27, 1, t);
        } else if (k.KKM(e, 16384, 0)) {
            m = 1;
            k.KO(0, t, "አ");
        } else if (k.KKM(e, 16384, 0) && k.KA(0, k.KC(1, 1, t), this.s15)) {
            m = 1;
            k.KIO(1, this.s14, 1, t);
        } else if (k.KKM(e, 16384, 0)) {
            m = 1;
            k.KO(0, t, "ኤ");
        } else if (k.KKM(e, 16384, 0)) {
            m = 1;
            k.KO(0, t, "ች");
        } else if (k.KKM(e, 16384, 0) && k.KCM(1, t, "አ", 1)) {
            m = 1;
            k.KO(1, t, "ኧ");
        } else if (k.KKM(e, 16384, 0) && k.KCM(1, t, "እ", 1)) {
            m = 1;
            k.KO(1, t, "ኧ");
        } else if (k.KKM(e, 16384, 0) && k.KCM(1, t, "ኢ", 1)) {
            m = 1;
            k.KO(1, t, "ኤ");
        } else if (k.KKM(e, 16384, 0) && k.KA(0, k.KC(1, 1, t), this.s15)) {
            m = 1;
            k.KIO(1, this.s10, 1, t);
        } else if (k.KKM(e, 16384, 0) && k.KA(0, k.KC(1, 1, t), this.s12)) {
            m = 1;
            k.KIO(1, this.s14, 1, t);
        } else if (k.KKM(e, 16384, 0) && k.KA(0, k.KC(1, 1, t), this.s28)) {
            m = 1;
            k.KIO(1, this.s29, 1, t);
        } else if (k.KKM(e, 16384, 0) && k.KA(0, k.KC(1, 1, t), this.s31)) {
            m = 1;
            k.KIO(1, this.s33, 1, t);
        } else if (k.KKM(e, 16384, 0)) {
            m = 1;
            k.KO(0, t, "እ");
        } else if (k.KKM(e, 16384, 0) && k.KCM(1, t, "አ", 1)) {
            m = 1;
            k.KO(1, t, "ኧ");
        } else if (k.KKM(e, 16384, 0) && k.KCM(1, t, "እ", 1)) {
            m = 1;
            k.KO(1, t, "ኧ");
        } else if (k.KKM(e, 16384, 0) && k.KCM(1, t, "ኢ", 1)) {
            m = 1;
            k.KO(1, t, "ኤ");
        } else if (k.KKM(e, 16384, 0) && k.KA(0, k.KC(1, 1, t), this.s15)) {
            m = 1;
            k.KIO(1, this.s10, 1, t);
        } else if (k.KKM(e, 16384, 0) && k.KA(0, k.KC(1, 1, t), this.s12)) {
            m = 1;
            k.KIO(1, this.s14, 1, t);
        } else if (k.KKM(e, 16384, 0) && k.KA(0, k.KC(1, 1, t), this.s28)) {
            m = 1;
            k.KIO(1, this.s29, 1, t);
        } else if (k.KKM(e, 16384, 0) && k.KA(0, k.KC(1, 1, t), this.s31)) {
            m = 1;
            k.KIO(1, this.s33, 1, t);
        } else if (k.KKM(e, 16384, 0)) {
            m = 1;
            k.KO(0, t, "እ");
        } else if (k.KKM(e, 16384, 0) && k.KCM(1, t, "አ", 1)) {
            m = 1;
            k.KO(1, t, "ኧ");
        } else if (k.KKM(e, 16384, 0) && k.KCM(1, t, "እ", 1)) {
            m = 1;
            k.KO(1, t, "ኧ");
        } else if (k.KKM(e, 16384, 0) && k.KCM(1, t, "ኢ", 1)) {
            m = 1;
            k.KO(1, t, "ኤ");
        } else if (k.KKM(e, 16384, 0) && k.KA(0, k.KC(1, 1, t), this.s15)) {
            m = 1;
            k.KIO(1, this.s10, 1, t);
        } else if (k.KKM(e, 16384, 0) && k.KA(0, k.KC(1, 1, t), this.s12)) {
            m = 1;
            k.KIO(1, this.s14, 1, t);
        } else if (k.KKM(e, 16384, 0) && k.KA(0, k.KC(1, 1, t), this.s28)) {
            m = 1;
            k.KIO(1, this.s29, 1, t);
        } else if (k.KKM(e, 16384, 0) && k.KA(0, k.KC(1, 1, t), this.s31)) {
            m = 1;
            k.KIO(1, this.s33, 1, t);
        } else if (k.KKM(e, 16384, 0)) {
            m = 1;
            k.KO(0, t, "እ");
        } else if (k.KKM(e, 16384, 0) && k.KCM(1, t, "አ", 1)) {
            m = 1;
            k.KO(1, t, "ኧ");
        } else if (k.KKM(e, 16384, 0) && k.KCM(1, t, "እ", 1)) {
            m = 1;
            k.KO(1, t, "ኧ");
        } else if (k.KKM(e, 16384, 0) && k.KCM(1, t, "ኢ", 1)) {
            m = 1;
            k.KO(1, t, "ኤ");
        } else if (k.KKM(e, 16384, 0) && k.KA(0, k.KC(1, 1, t), this.s15)) {
            m = 1;
            k.KIO(1, this.s10, 1, t);
        } else if (k.KKM(e, 16384, 0) && k.KA(0, k.KC(1, 1, t), this.s12)) {
            m = 1;
            k.KIO(1, this.s14, 1, t);
        } else if (k.KKM(e, 16384, 0) && k.KA(0, k.KC(1, 1, t), this.s28)) {
            m = 1;
            k.KIO(1, this.s29, 1, t);
        } else if (k.KKM(e, 16384, 0) && k.KA(0, k.KC(1, 1, t), this.s31)) {
            m = 1;
            k.KIO(1, this.s33, 1, t);
        } else if (k.KKM(e, 16384, 0)) {
            m = 1;
            k.KO(0, t, "እ");
        } else if (k.KKM(e, 16384, 0) && k.KCM(1, t, "እ", 1)) {
            m = 1;
            k.KO(1, t, "ኢ");
        } else if (k.KKM(e, 16384, 0) && k.KA(0, k.KC(1, 1, t), this.s15)) {
            m = 1;
            k.KIO(1, this.s12, 1, t);
        } else if (k.KKM(e, 16384, 0) && k.KA(0, k.KC(1, 1, t), this.s28)) {
            m = 1;
            k.KIO(1, this.s31, 1, t);
        } else if (k.KKM(e, 16384, 0)) {
            m = 1;
            k.KO(0, t, "ኢ");
        } else if (k.KKM(e, 16384, 0) && k.KCM(1, t, "እ", 1)) {
            m = 1;
            k.KO(1, t, "ኢ");
        } else if (k.KKM(e, 16384, 0) && k.KA(0, k.KC(1, 1, t), this.s15)) {
            m = 1;
            k.KIO(1, this.s12, 1, t);
        } else if (k.KKM(e, 16384, 0) && k.KA(0, k.KC(1, 1, t), this.s28)) {
            m = 1;
            k.KIO(1, this.s31, 1, t);
        } else if (k.KKM(e, 16384, 0)) {
            m = 1;
            k.KO(0, t, "ኢ");
        } else if (k.KKM(e, 16384, 0) && k.KCM(1, t, "እ", 1)) {
            m = 1;
            k.KO(1, t, "ኢ");
        } else if (k.KKM(e, 16384, 0) && k.KA(0, k.KC(1, 1, t), this.s15)) {
            m = 1;
            k.KIO(1, this.s12, 1, t);
        } else if (k.KKM(e, 16384, 0) && k.KA(0, k.KC(1, 1, t), this.s28)) {
            m = 1;
            k.KIO(1, this.s31, 1, t);
        } else if (k.KKM(e, 16384, 0)) {
            m = 1;
            k.KO(0, t, "ኢ");
        } else if (k.KKM(e, 16384, 0) && k.KCM(1, t, "እ", 1)) {
            m = 1;
            k.KO(1, t, "ኢ");
        } else if (k.KKM(e, 16384, 0) && k.KA(0, k.KC(1, 1, t), this.s15)) {
            m = 1;
            k.KIO(1, this.s12, 1, t);
        } else if (k.KKM(e, 16384, 0) && k.KA(0, k.KC(1, 1, t), this.s28)) {
            m = 1;
            k.KIO(1, this.s31, 1, t);
        } else if (k.KKM(e, 16384, 0)) {
            m = 1;
            k.KO(0, t, "ኢ");
        } else if (k.KKM(e, 16384, 0)) {
            m = 1;
            k.KO(0, t, "ድ");
        } else if (k.KKM(e, 16384, 0)) {
            m = 1;
            k.KO(0, t, "ኝ");
        } else if (k.KKM(e, 16384, 0) && k.KCM(1, t, "እ", 1)) {
            m = 1;
            k.KO(1, t, "ኦ");
        } else if (k.KKM(e, 16384, 0) && k.KA(0, k.KC(1, 1, t), this.s15)) {
            m = 1;
            k.KIO(1, this.s16, 1, t);
        } else if (k.KKM(e, 16384, 0)) {
            m = 1;
            k.KO(0, t, "ኦ");
        } else if (k.KKM(e, 16384, 0) && k.KCM(1, t, "እ", 1)) {
            m = 1;
            k.KO(1, t, "ኦ");
        } else if (k.KKM(e, 16384, 0) && k.KA(0, k.KC(1, 1, t), this.s15)) {
            m = 1;
            k.KIO(1, this.s16, 1, t);
        } else if (k.KKM(e, 16384, 0)) {
            m = 1;
            k.KO(0, t, "ኦ");
        } else if (k.KKM(e, 16384, 0) && k.KCM(1, t, "እ", 1)) {
            m = 1;
            k.KO(1, t, "ኦ");
        } else if (k.KKM(e, 16384, 0) && k.KA(0, k.KC(1, 1, t), this.s15)) {
            m = 1;
            k.KIO(1, this.s16, 1, t);
        } else if (k.KKM(e, 16384, 0)) {
            m = 1;
            k.KO(0, t, "ኦ");
        } else if (k.KKM(e, 16384, 0) && k.KCM(1, t, "እ", 1)) {
            m = 1;
            k.KO(1, t, "ኦ");
        } else if (k.KKM(e, 16384, 0) && k.KA(0, k.KC(1, 1, t), this.s15)) {
            m = 1;
            k.KIO(1, this.s16, 1, t);
        } else if (k.KKM(e, 16384, 0)) {
            m = 1;
            k.KO(0, t, "ኦ");
        } else if (k.KKM(e, 16384, 0) && k.KCM(1, t, "እ", 1)) {
            m = 1;
            k.KO(1, t, "ኦ");
        } else if (k.KKM(e, 16384, 0) && k.KA(0, k.KC(1, 1, t), this.s15)) {
            m = 1;
            k.KIO(1, this.s16, 1, t);
        } else if (k.KKM(e, 16384, 0)) {
            m = 1;
            k.KO(0, t, "ኦ");
        } else if (k.KKM(e, 16384, 0) && k.KCM(1, t, "እ", 1)) {
            m = 1;
            k.KO(1, t, "ኦ");
        } else if (k.KKM(e, 16384, 0) && k.KA(0, k.KC(1, 1, t), this.s15)) {
            m = 1;
            k.KIO(1, this.s16, 1, t);
        } else if (k.KKM(e, 16384, 0)) {
            m = 1;
            k.KO(0, t, "ኦ");
        } else if (k.KKM(e, 16384, 0) && k.KCM(1, t, "እ", 1)) {
            m = 1;
            k.KO(1, t, "ኡ");
        } else if (k.KKM(e, 16384, 0) && k.KA(0, k.KC(1, 1, t), this.s15)) {
            m = 1;
            k.KIO(1, this.s11, 1, t);
        } else if (k.KKM(e, 16384, 0) && k.KA(0, k.KC(1, 1, t), this.s28)) {
            m = 1;
            k.KIO(1, this.s30, 1, t);
        } else if (k.KKM(e, 16384, 0)) {
            m = 1;
            k.KO(0, t, "ኡ");
        } else if (k.KKM(e, 16384, 0) && k.KCM(1, t, "እ", 1)) {
            m = 1;
            k.KO(1, t, "ኡ");
        } else if (k.KKM(e, 16384, 0) && k.KA(0, k.KC(1, 1, t), this.s15)) {
            m = 1;
            k.KIO(1, this.s11, 1, t);
        } else if (k.KKM(e, 16384, 0) && k.KA(0, k.KC(1, 1, t), this.s28)) {
            m = 1;
            k.KIO(1, this.s30, 1, t);
        } else if (k.KKM(e, 16384, 0)) {
            m = 1;
            k.KO(0, t, "ኡ");
        } else if (k.KKM(e, 16384, 0) && k.KCM(1, t, "እ", 1)) {
            m = 1;
            k.KO(1, t, "ኡ");
        } else if (k.KKM(e, 16384, 0) && k.KA(0, k.KC(1, 1, t), this.s15)) {
            m = 1;
            k.KIO(1, this.s11, 1, t);
        } else if (k.KKM(e, 16384, 0) && k.KA(0, k.KC(1, 1, t), this.s28)) {
            m = 1;
            k.KIO(1, this.s30, 1, t);
        } else if (k.KKM(e, 16384, 0)) {
            m = 1;
            k.KO(0, t, "ኡ");
        } else if (k.KKM(e, 16384, 0) && k.KCM(1, t, "እ", 1)) {
            m = 1;
            k.KO(1, t, "ኡ");
        } else if (k.KKM(e, 16384, 0) && k.KA(0, k.KC(1, 1, t), this.s15)) {
            m = 1;
            k.KIO(1, this.s11, 1, t);
        } else if (k.KKM(e, 16384, 0) && k.KA(0, k.KC(1, 1, t), this.s28)) {
            m = 1;
            k.KIO(1, this.s30, 1, t);
        } else if (k.KKM(e, 16384, 0)) {
            m = 1;
            k.KO(0, t, "ኡ");
        } else if (k.KKM(e, 16384, 0)) {
            m = 1;
            k.KO(0, t, "ይ");
        } else if (k.KKM(e, 16400, 32)) {
            m = 1;
            k.KO(0, t, "ይ");
        }
        return m;
    };
}
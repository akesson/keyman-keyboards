KeymanWeb.KR(new Keyboard_greek_polytonic());

function Keyboard_greek_polytonic() {
    this.KI = "Keyboard_greek_polytonic";
    this.KBVER = '1.1';
    this.KN = "Greek Polytonic";
    this.KV = {
        F: ' 1em "Tahoma"',
        K102: 1,
        KDU: 1,
        BK: new Array("ΐ ", "1", "2", "3", "4", "5", "6", "7", "8", "9", "0", "ᾱ", "ἆ", "", "", "", "ά", "ς", "ε", "ρ", "τ", "υ", "θ", "ι", "ο", "π", "ᾶ", "ὰ", "ἂ", "", "", "", "α", "σ", "δ", "φ", "γ", "η", "ξ", "κ", "λ", "ά", "ἀ", "", "", "", "", "", "<", "ζ", "χ", "ψ", "ω", "β", "ν", "μ", ",", ".", "ἄ", "", "", "", "", "", " ", "", "!", "@", "#", "$", "%", "^", "&", "*", "(", ")", "ᾰ", "ἇ", "", "", "", "", "", "Ε", "Ρ", "Τ", "Υ", "Θ", "Ι", "Ο", "Π", "ᾳ", "", "ἃ", "", "", "", "Α", "Σ", "Δ", "Φ", "Γ", "Η", "Ξ", "Κ", "Λ", "", "ἁ", "", "", "", "", "", ">", "Ζ", "Χ", "Ψ", "Ω", "Β", "Ν", "Μ", "<", ">", "ἅ", "", "", "", "", "", " ", "", "", "", "", "", "", "", "", "", "", "", "", "", "", "", "", "", "", "", "", "", "", "", "", "", "", "", "", "", "", "", "", "", "", "", "", "", "", "", "", "", "", "", "", "", "", "", "", "", "", "", "", "", "", "", "", "", "", "", "", "", "", "", "", " ", "", "", "", "", "", "", "", "", "", "", "", "", "", "", "", "", "", "", "", "", "", "", "", "", "", "", "", "", "", "", "", "", "", "", "", "", "", "", "", "", "", "", "", "", "", "", "", "", "", "", "", "", "", "", "", "", "", "", "", "", "", "", "", "", "", "", "", "", "", "", "", "", "", "", "", "", "", "", "", "", "", "", "", "", "", "", "", "", "", "", "", "", "", "", "", "", "", "", "", "", "", "", "", "", "", "", "", "", "", "", "", "", "", "", "", "", "", "", "", "", "", "", "", "", "", "", "", "", "", "", "", "", "", "", "", "", "", "", "", "", "", "", "", "", "", "", "", "", "", "", "", "", "", "", "", "", "", "", "", "", "", "", "", "", "", "", "", "", "", "", "", "", "", "", "", "", "", "", "", "", "", "", "", "", "", "", "", "", "", "", "", "", "", "", "", "", "Ϛ", "Ϟ", "Ϡ", "£", "§", "¶", "", "¤", "¦", "°", "±", "", "", "", "", "", "", "€", "®", "", "¥", "", "", "", "", "", "", "", "", "", "", "", "", "", "", "", "", "", "", "", "", "", "", "", "", "", "", "", "", "", "©", "", "", "", "", "", "", "", "", "", "", "", "", "", "", "", "²", "³", "", "", "", "", "", "", "", "", "", "", "", "", "", "", "", "", "", "", "", "", "", "", "", "·", "", "", "", "", "", "", "", "", "", "", "", "", "", "", "", "", "", "", "", "", "", "", "", "", "", "", "", "", "", "", "", "", "", "", "", "", "")
    };
    this.KH = '';
    this.KM = 0;
    this.s9 = " αΑεΕηΗιΙοΟωΩυΥ.";
    this.s10 = ";άΆέΈήΉίΊόΌώΏύΎ´";
    this.s11 = " ιΙυΥ.";
    this.s12 = ":ϊΪϋΫ¨";
    this.s13 = "αηω";
    this.s14 = "ᾴῄῴ";
    this.s15 = " ιυ.";
    this.s16 = "΅ΐΰ΅";
    this.s17 = " αΑεΕηΗιΙοΟωΩυΥ.";
    this.s18 = "΄άΆέΈήΉίΊόΌώΏύΎ΄";
    this.s19 = " ιΙυΥ.";
    this.s20 = "¨ϊΪϋΫ¨";
    this.s21 = " αΑηΗιΙωΩυ.";
    this.s22 = "=ἆἎἦἮἶἾὦὮὖ῏";
    this.s23 = " αΑηΗιΙωΩυΥ.";
    this.s24 = "+ἇἏἧἯἷἿὧὯὗὟ῟";
    this.s25 = " αΑηΗωΩ";
    this.s26 = "½ᾆᾎᾖᾞᾦᾮ";
    this.s27 = "αΑηΗωΩ";
    this.s28 = "ᾇᾏᾗᾟᾧᾯ";
    this.s29 = " αΑιΙυΥ.";
    this.s30 = "-ᾱᾹῑῙῡῩ¯";
    this.s31 = " αΑιΙυΥ.";
    this.s32 = "_ᾰᾸῐῘῠῨ˘";
    this.s33 = " αΑεΕηΗιΙοΟωΩυ.";
    this.s34 = "/ἄἌἔἜἤἬἴἼὄὌὤὬὔ῎";
    this.s35 = " αΑεΕηΗιΙοΟωΩυΥ.";
    this.s36 = "?ἅἍἕἝἥἭἵἽὅὍὥὭὕὝ῞";
    this.s37 = "αΑηΗωΩ";
    this.s38 = "ᾄᾌᾔᾜᾤᾬ";
    this.s39 = "αΑηΗωΩ";
    this.s40 = "ᾅᾍᾕᾝᾥᾭ";
    this.s41 = " ιυ.";
    this.s42 = "~ΐΰ΅";
    this.s43 = " ιυ.";
    this.s44 = "`ῒῢ῭";
    this.s45 = " ιυ.";
    this.s46 = "῁ῗῧ῁";
    this.s47 = " αηιωυ.";
    this.s48 = "[ᾶῆῖῶῦ῀";
    this.s49 = " αΑηΗωΩ.";
    this.s50 = "{ᾳᾼῃῌῳῼι";
    this.s51 = " αηω";
    this.s52 = "«ᾷῇῷ";
    this.s53 = " αΑεΕηΗιΙοΟωΩυ.";
    this.s54 = "\\ἂἊἒἚἢἪἲἺὂὊὢὪὒ῍";
    this.s55 = " αΑεΕηΗιΙοΟωΩυΥ.";
    this.s56 = "|ἃἋἓἛἣἫἳἻὃὋὣὫὓὛ῝";
    this.s57 = " αΑηΗωΩ";
    this.s58 = "¬ᾂᾊᾒᾚᾢᾪ";
    this.s59 = "αΑηΗωΩ";
    this.s60 = "ᾃᾋᾓᾛᾣᾫ";
    this.s61 = " αΑεΕηΗιΙοΟωΩυΥ.";
    this.s62 = "]ὰᾺὲῈὴῊὶῚὸῸὼῺὺῪ`";
    this.s63 = " .";
    this.s64 = "}·";
    this.s65 = " αηω";
    this.s66 = "»ᾲῂῲ";
    this.s67 = " αΑεΕηΗιΙοΟρωΩυ.";
    this.s68 = "'ἀἈἐἘἠἨἰἸὀὈῤὠὨὐ᾿";
    this.s69 = " αΑεΕηΗιΙοΟρΡωΩυΥ.";
    this.s70 = "\"ἁἉἑἙἡἩἱἹὁὉῥῬὡὩὑὙ῾";
    this.s71 = "αΑηΗωΩ";
    this.s72 = "ᾀᾈᾐᾘᾠᾨ";
    this.s73 = "αΑηΗωΩ";
    this.s74 = "ᾁᾉᾑᾙᾡᾩ";
    this.KVER = "9.0.424.0";
    this.gs = function(t, e) {
        return this.g0(t, e);
    };
    this.g0 = function(t, e) {
        var k = KeymanWeb,
            r = 0,
            m = 0;
        if (k.KKM(e, 16384, 32)) {
            r = m = 1;
            k.KO(0, t, " ");
        } else if (k.KKM(e, 16400, 32)) {
            r = m = 1;
            k.KO(0, t, " ");
        } else if (k.KKM(e, 16416, 32)) {
            r = m = 1;
            k.KO(0, t, " ");
        } else if (k.KKM(e, 16384, 48)) {
            r = m = 1;
            k.KO(0, t, "0");
        } else if (k.KKM(e, 16400, 48)) {
            r = m = 1;
            k.KO(0, t, ")");
        } else if (k.KKM(e, 16480, 48)) {
            r = m = 1;
            k.KO(0, t, "°");
        } else if (k.KKM(e, 16384, 49)) {
            r = m = 1;
            k.KO(0, t, "1");
        } else if (k.KKM(e, 16400, 49)) {
            r = m = 1;
            k.KO(0, t, "!");
        } else if (k.KKM(e, 16480, 49)) {
            r = m = 1;
            k.KO(0, t, "Ϛ");
        } else if (k.KKM(e, 16384, 50)) {
            r = m = 1;
            k.KO(0, t, "2");
        } else if (k.KKM(e, 16400, 50)) {
            r = m = 1;
            k.KO(0, t, "@");
        } else if (k.KKM(e, 16480, 50)) {
            r = m = 1;
            k.KO(0, t, "Ϟ");
        } else if (k.KKM(e, 16496, 50)) {
            r = m = 1;
            k.KO(0, t, "²");
        } else if (k.KKM(e, 16384, 51)) {
            r = m = 1;
            k.KO(0, t, "3");
        } else if (k.KKM(e, 16400, 51)) {
            r = m = 1;
            k.KO(0, t, "#");
        } else if (k.KKM(e, 16480, 51)) {
            r = m = 1;
            k.KO(0, t, "Ϡ");
        } else if (k.KKM(e, 16496, 51)) {
            r = m = 1;
            k.KO(0, t, "³");
        } else if (k.KKM(e, 16384, 52)) {
            r = m = 1;
            k.KO(0, t, "4");
        } else if (k.KKM(e, 16400, 52)) {
            r = m = 1;
            k.KO(0, t, "$");
        } else if (k.KKM(e, 16480, 52)) {
            r = m = 1;
            k.KO(0, t, "£");
        } else if (k.KKM(e, 16384, 53)) {
            r = m = 1;
            k.KO(0, t, "5");
        } else if (k.KKM(e, 16400, 53)) {
            r = m = 1;
            k.KO(0, t, "%");
        } else if (k.KKM(e, 16480, 53)) {
            r = m = 1;
            k.KO(0, t, "§");
        } else if (k.KKM(e, 16384, 54)) {
            r = m = 1;
            k.KO(0, t, "6");
        } else if (k.KKM(e, 16400, 54)) {
            r = m = 1;
            k.KO(0, t, "^");
        } else if (k.KKM(e, 16480, 54)) {
            r = m = 1;
            k.KO(0, t, "¶");
        } else if (k.KKM(e, 16384, 55)) {
            r = m = 1;
            k.KO(0, t, "7");
        } else if (k.KKM(e, 16400, 55)) {
            r = m = 1;
            k.KO(0, t, "&");
        } else if (k.KKM(e, 16384, 56)) {
            r = m = 1;
            k.KO(0, t, "8");
        } else if (k.KKM(e, 16400, 56)) {
            r = m = 1;
            k.KO(0, t, "*");
        } else if (k.KKM(e, 16480, 56)) {
            r = m = 1;
            k.KO(0, t, "¤");
        } else if (k.KKM(e, 16384, 57)) {
            r = m = 1;
            k.KO(0, t, "9");
        } else if (k.KKM(e, 16400, 57)) {
            r = m = 1;
            k.KO(0, t, "(");
        } else if (k.KKM(e, 16480, 57)) {
            r = m = 1;
            k.KO(0, t, "¦");
        } else if (k.KKM(e, 16384, 65)) {
            r = m = 1;
            k.KO(0, t, "α");
        } else if (k.KKM(e, 16400, 65)) {
            r = m = 1;
            k.KO(0, t, "Α");
        } else if (k.KKM(e, 16384, 66)) {
            r = m = 1;
            k.KO(0, t, "β");
        } else if (k.KKM(e, 16400, 66)) {
            r = m = 1;
            k.KO(0, t, "Β");
        } else if (k.KKM(e, 16384, 67)) {
            r = m = 1;
            k.KO(0, t, "ψ");
        } else if (k.KKM(e, 16400, 67)) {
            r = m = 1;
            k.KO(0, t, "Ψ");
        } else if (k.KKM(e, 16480, 67)) {
            r = m = 1;
            k.KO(0, t, "©");
        } else if (k.KKM(e, 16384, 68)) {
            r = m = 1;
            k.KO(0, t, "δ");
        } else if (k.KKM(e, 16400, 68)) {
            r = m = 1;
            k.KO(0, t, "Δ");
        } else if (k.KKM(e, 16384, 69)) {
            r = m = 1;
            k.KO(0, t, "ε");
        } else if (k.KKM(e, 16400, 69)) {
            r = m = 1;
            k.KO(0, t, "Ε");
        } else if (k.KKM(e, 16480, 69)) {
            r = m = 1;
            k.KO(0, t, "€");
        } else if (k.KKM(e, 16384, 70)) {
            r = m = 1;
            k.KO(0, t, "φ");
        } else if (k.KKM(e, 16400, 70)) {
            r = m = 1;
            k.KO(0, t, "Φ");
        } else if (k.KKM(e, 16384, 71)) {
            r = m = 1;
            k.KO(0, t, "γ");
        } else if (k.KKM(e, 16400, 71)) {
            r = m = 1;
            k.KO(0, t, "Γ");
        } else if (k.KKM(e, 16384, 72)) {
            r = m = 1;
            k.KO(0, t, "η");
        } else if (k.KKM(e, 16400, 72)) {
            r = m = 1;
            k.KO(0, t, "Η");
        } else if (k.KKM(e, 16384, 73)) {
            r = m = 1;
            k.KO(0, t, "ι");
        } else if (k.KKM(e, 16400, 73)) {
            r = m = 1;
            k.KO(0, t, "Ι");
        } else if (k.KKM(e, 16384, 74)) {
            r = m = 1;
            k.KO(0, t, "ξ");
        } else if (k.KKM(e, 16400, 74)) {
            r = m = 1;
            k.KO(0, t, "Ξ");
        } else if (k.KKM(e, 16384, 75)) {
            r = m = 1;
            k.KO(0, t, "κ");
        } else if (k.KKM(e, 16400, 75)) {
            r = m = 1;
            k.KO(0, t, "Κ");
        } else if (k.KKM(e, 16384, 76)) {
            r = m = 1;
            k.KO(0, t, "λ");
        } else if (k.KKM(e, 16400, 76)) {
            r = m = 1;
            k.KO(0, t, "Λ");
        } else if (k.KKM(e, 16384, 77)) {
            r = m = 1;
            k.KO(0, t, "μ");
        } else if (k.KKM(e, 16400, 77)) {
            r = m = 1;
            k.KO(0, t, "Μ");
        } else if (k.KKM(e, 16384, 78)) {
            r = m = 1;
            k.KO(0, t, "ν");
        } else if (k.KKM(e, 16400, 78)) {
            r = m = 1;
            k.KO(0, t, "Ν");
        } else if (k.KKM(e, 16384, 79)) {
            r = m = 1;
            k.KO(0, t, "ο");
        } else if (k.KKM(e, 16400, 79)) {
            r = m = 1;
            k.KO(0, t, "Ο");
        } else if (k.KKM(e, 16384, 80)) {
            r = m = 1;
            k.KO(0, t, "π");
        } else if (k.KKM(e, 16400, 80)) {
            r = m = 1;
            k.KO(0, t, "Π");
        } else if (k.KKM(e, 16384, 81)) {
            r = m = 1;
            k.KDO(0, t, 0);
        } else if (k.KKM(e, 16400, 81)) {
            r = m = 1;
            k.KDO(0, t, 1);
        } else if (k.KKM(e, 16480, 81)) {
            r = m = 1;
            k.KDO(0, t, 2);
        } else if (k.KKM(e, 16384, 82)) {
            r = m = 1;
            k.KO(0, t, "ρ");
        } else if (k.KKM(e, 16400, 82)) {
            r = m = 1;
            k.KO(0, t, "Ρ");
        } else if (k.KKM(e, 16480, 82)) {
            r = m = 1;
            k.KO(0, t, "®");
        } else if (k.KKM(e, 16384, 83)) {
            r = m = 1;
            k.KO(0, t, "σ");
        } else if (k.KKM(e, 16400, 83)) {
            r = m = 1;
            k.KO(0, t, "Σ");
        } else if (k.KKM(e, 16384, 84)) {
            r = m = 1;
            k.KO(0, t, "τ");
        } else if (k.KKM(e, 16400, 84)) {
            r = m = 1;
            k.KO(0, t, "Τ");
        } else if (k.KKM(e, 16384, 85)) {
            r = m = 1;
            k.KO(0, t, "θ");
        } else if (k.KKM(e, 16400, 85)) {
            r = m = 1;
            k.KO(0, t, "Θ");
        } else if (k.KKM(e, 16384, 86)) {
            r = m = 1;
            k.KO(0, t, "ω");
        } else if (k.KKM(e, 16400, 86)) {
            r = m = 1;
            k.KO(0, t, "Ω");
        } else if (k.KKM(e, 16384, 87)) {
            r = m = 1;
            k.KO(0, t, "ς");
        } else if (k.KKM(e, 16400, 87)) {
            r = m = 1;
            k.KDO(0, t, 3);
        } else if (k.KKM(e, 16384, 88)) {
            r = m = 1;
            k.KO(0, t, "χ");
        } else if (k.KKM(e, 16400, 88)) {
            r = m = 1;
            k.KO(0, t, "Χ");
        } else if (k.KKM(e, 16384, 89)) {
            r = m = 1;
            k.KO(0, t, "υ");
        } else if (k.KKM(e, 16400, 89)) {
            r = m = 1;
            k.KO(0, t, "Υ");
        } else if (k.KKM(e, 16480, 89)) {
            r = m = 1;
            k.KO(0, t, "¥");
        } else if (k.KKM(e, 16384, 90)) {
            r = m = 1;
            k.KO(0, t, "ζ");
        } else if (k.KKM(e, 16400, 90)) {
            r = m = 1;
            k.KO(0, t, "Ζ");
        } else if (k.KKM(e, 16384, 186)) {
            r = m = 1;
            k.KDO(0, t, 4);
        } else if (k.KKM(e, 16400, 186)) {
            r = m = 1;
            k.KDO(0, t, 5);
        } else if (k.KKM(e, 16480, 186)) {
            r = m = 1;
            k.KDO(0, t, 3);
        } else if (k.KKM(e, 16384, 187)) {
            r = m = 1;
            k.KDO(0, t, 6);
        } else if (k.KKM(e, 16400, 187)) {
            r = m = 1;
            k.KDO(0, t, 7);
        } else if (k.KKM(e, 16480, 187)) {
            r = m = 1;
            k.KDO(0, t, 8);
        } else if (k.KKM(e, 16496, 187)) {
            r = m = 1;
            k.KDO(0, t, 9);
        } else if (k.KKM(e, 16384, 188)) {
            r = m = 1;
            k.KO(0, t, ",");
        } else if (k.KKM(e, 16400, 188)) {
            r = m = 1;
            k.KO(0, t, "<");
        } else if (k.KKM(e, 16384, 189)) {
            r = m = 1;
            k.KDO(0, t, 10);
        } else if (k.KKM(e, 16400, 189)) {
            r = m = 1;
            k.KDO(0, t, 11);
        } else if (k.KKM(e, 16480, 189)) {
            r = m = 1;
            k.KO(0, t, "±");
        } else if (k.KKM(e, 16384, 190)) {
            r = m = 1;
            k.KO(0, t, ".");
        } else if (k.KKM(e, 16400, 190)) {
            r = m = 1;
            k.KO(0, t, ">");
        } else if (k.KKM(e, 16384, 191)) {
            r = m = 1;
            k.KDO(0, t, 12);
        } else if (k.KKM(e, 16400, 191)) {
            r = m = 1;
            k.KDO(0, t, 13);
        } else if (k.KKM(e, 16480, 191)) {
            r = m = 1;
            k.KDO(0, t, 14);
        } else if (k.KKM(e, 16496, 191)) {
            r = m = 1;
            k.KDO(0, t, 15);
        } else if (k.KKM(e, 16384, 192)) {
            r = m = 1;
            k.KDO(0, t, 16);
        } else if (k.KKM(e, 16400, 192)) {
            r = m = 1;
            k.KDO(0, t, 17);
        } else if (k.KKM(e, 16480, 192)) {
            r = m = 1;
            k.KDO(0, t, 18);
        } else if (k.KKM(e, 16384, 219)) {
            r = m = 1;
            k.KDO(0, t, 19);
        } else if (k.KKM(e, 16400, 219)) {
            r = m = 1;
            k.KDO(0, t, 20);
        } else if (k.KKM(e, 16480, 219)) {
            r = m = 1;
            k.KDO(0, t, 21);
        } else if (k.KKM(e, 16384, 220)) {
            r = m = 1;
            k.KDO(0, t, 22);
        } else if (k.KKM(e, 16400, 220)) {
            r = m = 1;
            k.KDO(0, t, 23);
        } else if (k.KKM(e, 16480, 220)) {
            r = m = 1;
            k.KDO(0, t, 24);
        } else if (k.KKM(e, 16496, 220)) {
            r = m = 1;
            k.KDO(0, t, 25);
        } else if (k.KKM(e, 16384, 221)) {
            r = m = 1;
            k.KDO(0, t, 26);
        } else if (k.KKM(e, 16400, 221)) {
            r = m = 1;
            k.KDO(0, t, 27);
        } else if (k.KKM(e, 16480, 221)) {
            r = m = 1;
            k.KDO(0, t, 28);
        } else if (k.KKM(e, 16496, 221)) {
            r = m = 1;
            k.KO(0, t, "·");
        } else if (k.KKM(e, 16384, 222)) {
            r = m = 1;
            k.KDO(0, t, 29);
        } else if (k.KKM(e, 16400, 222)) {
            r = m = 1;
            k.KDO(0, t, 30);
        } else if (k.KKM(e, 16480, 222)) {
            r = m = 1;
            k.KDO(0, t, 31);
        } else if (k.KKM(e, 16496, 222)) {
            r = m = 1;
            k.KDO(0, t, 32);
        } else if (k.KKM(e, 16384, 226)) {
            r = m = 1;
            k.KO(0, t, "<");
        } else if (k.KKM(e, 16400, 226)) {
            r = m = 1;
            k.KO(0, t, ">");
        }
        if (m) {
            r = this.g1(t, e);
        }
        return r;
    };
    this.g1 = function(t, e) {
        var k = KeymanWeb,
            r = 1,
            m = 0;
        if (k.KDM(1, t, 0) && k.KA(0, k.KC(1, 1, t), this.s9)) {
            m = 1;
            k.KIO(1, this.s10, 1, t);
        } else if (k.KDM(1, t, 1) && k.KA(0, k.KC(1, 1, t), this.s11)) {
            m = 1;
            k.KIO(1, this.s12, 1, t);
        } else if (k.KDM(1, t, 2) && k.KA(0, k.KC(1, 1, t), this.s13)) {
            m = 1;
            k.KIO(1, this.s14, 1, t);
        } else if (k.KDM(1, t, 3) && k.KA(0, k.KC(1, 1, t), this.s15)) {
            m = 1;
            k.KIO(1, this.s16, 1, t);
        } else if (k.KDM(1, t, 4) && k.KA(0, k.KC(1, 1, t), this.s17)) {
            m = 1;
            k.KIO(1, this.s18, 1, t);
        } else if (k.KDM(1, t, 5) && k.KA(0, k.KC(1, 1, t), this.s19)) {
            m = 1;
            k.KIO(1, this.s20, 1, t);
        } else if (k.KDM(1, t, 6) && k.KA(0, k.KC(1, 1, t), this.s21)) {
            m = 1;
            k.KIO(1, this.s22, 1, t);
        } else if (k.KDM(1, t, 7) && k.KA(0, k.KC(1, 1, t), this.s23)) {
            m = 1;
            k.KIO(1, this.s24, 1, t);
        } else if (k.KDM(1, t, 8) && k.KA(0, k.KC(1, 1, t), this.s25)) {
            m = 1;
            k.KIO(1, this.s26, 1, t);
        } else if (k.KDM(1, t, 9) && k.KA(0, k.KC(1, 1, t), this.s27)) {
            m = 1;
            k.KIO(1, this.s28, 1, t);
        } else if (k.KDM(1, t, 10) && k.KA(0, k.KC(1, 1, t), this.s29)) {
            m = 1;
            k.KIO(1, this.s30, 1, t);
        } else if (k.KDM(1, t, 11) && k.KA(0, k.KC(1, 1, t), this.s31)) {
            m = 1;
            k.KIO(1, this.s32, 1, t);
        } else if (k.KDM(1, t, 12) && k.KA(0, k.KC(1, 1, t), this.s33)) {
            m = 1;
            k.KIO(1, this.s34, 1, t);
        } else if (k.KDM(1, t, 13) && k.KA(0, k.KC(1, 1, t), this.s35)) {
            m = 1;
            k.KIO(1, this.s36, 1, t);
        } else if (k.KDM(1, t, 14) && k.KA(0, k.KC(1, 1, t), this.s37)) {
            m = 1;
            k.KIO(1, this.s38, 1, t);
        } else if (k.KDM(1, t, 15) && k.KA(0, k.KC(1, 1, t), this.s39)) {
            m = 1;
            k.KIO(1, this.s40, 1, t);
        } else if (k.KDM(1, t, 16) && k.KA(0, k.KC(1, 1, t), this.s41)) {
            m = 1;
            k.KIO(1, this.s42, 1, t);
        } else if (k.KDM(1, t, 17) && k.KA(0, k.KC(1, 1, t), this.s43)) {
            m = 1;
            k.KIO(1, this.s44, 1, t);
        } else if (k.KDM(1, t, 18) && k.KA(0, k.KC(1, 1, t), this.s45)) {
            m = 1;
            k.KIO(1, this.s46, 1, t);
        } else if (k.KDM(1, t, 19) && k.KA(0, k.KC(1, 1, t), this.s47)) {
            m = 1;
            k.KIO(1, this.s48, 1, t);
        } else if (k.KDM(1, t, 20) && k.KA(0, k.KC(1, 1, t), this.s49)) {
            m = 1;
            k.KIO(1, this.s50, 1, t);
        } else if (k.KDM(1, t, 21) && k.KA(0, k.KC(1, 1, t), this.s51)) {
            m = 1;
            k.KIO(1, this.s52, 1, t);
        } else if (k.KDM(1, t, 22) && k.KA(0, k.KC(1, 1, t), this.s53)) {
            m = 1;
            k.KIO(1, this.s54, 1, t);
        } else if (k.KDM(1, t, 23) && k.KA(0, k.KC(1, 1, t), this.s55)) {
            m = 1;
            k.KIO(1, this.s56, 1, t);
        } else if (k.KDM(1, t, 24) && k.KA(0, k.KC(1, 1, t), this.s57)) {
            m = 1;
            k.KIO(1, this.s58, 1, t);
        } else if (k.KDM(1, t, 25) && k.KA(0, k.KC(1, 1, t), this.s59)) {
            m = 1;
            k.KIO(1, this.s60, 1, t);
        } else if (k.KDM(1, t, 26) && k.KA(0, k.KC(1, 1, t), this.s61)) {
            m = 1;
            k.KIO(1, this.s62, 1, t);
        } else if (k.KDM(1, t, 27) && k.KA(0, k.KC(1, 1, t), this.s63)) {
            m = 1;
            k.KIO(1, this.s64, 1, t);
        } else if (k.KDM(1, t, 28) && k.KA(0, k.KC(1, 1, t), this.s65)) {
            m = 1;
            k.KIO(1, this.s66, 1, t);
        } else if (k.KDM(1, t, 29) && k.KA(0, k.KC(1, 1, t), this.s67)) {
            m = 1;
            k.KIO(1, this.s68, 1, t);
        } else if (k.KDM(1, t, 30) && k.KA(0, k.KC(1, 1, t), this.s69)) {
            m = 1;
            k.KIO(1, this.s70, 1, t);
        } else if (k.KDM(1, t, 31) && k.KA(0, k.KC(1, 1, t), this.s71)) {
            m = 1;
            k.KIO(1, this.s72, 1, t);
        } else if (k.KDM(1, t, 32) && k.KA(0, k.KC(1, 1, t), this.s73)) {
            m = 1;
            k.KIO(1, this.s74, 1, t);
        }
        return r;
    };
}